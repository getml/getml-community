    ...
    licensing::LicenseChecker& _license_checker )
{
...

    {
        ...
        Thrower::throw_if(
            !file.exists(), "File or directory '" + path + "' not found!" );
    ...
    if ( _license_checker.token().mem_size_ > 0 )
        {
...
{
    if ( autosql::Endianness::is_little_endian() )
        {
    ...
    else
        {
            ...
            if ( Poco::File( _path + "join_keys_encoding" ).exists() )
                {
                    _join_keys_encodings = FileHandler::read_strings_big_endian(
                        _path + "join_keys_encoding" );
                }
        }
}
