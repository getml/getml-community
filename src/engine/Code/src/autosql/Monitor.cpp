...
bool Monitor::get_start_message() const
{
    ...
    const std::string url =
        "127.0.0.1:" + std::to_string( options_.monitor.port );
...
{
    if ( options_.monitor.tls_encryption )
        {
            ...
            Poco::Net::HTTPSClientSession session(
                "127.0.0.1", options_.monitor.port, context );
    ...
    else
        {
            Poco::Net::HTTPClientSession session(
                "127.0.0.1", options_.monitor.port );
