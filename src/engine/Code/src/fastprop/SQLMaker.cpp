    ...
    switch ( _condition.data_used_ )
        {
            case enums::DataUsed::categorical:
                {
                    ...
                    assert_true(
                        _condition.category_used_ < categories().size() );

                    const auto category =
                        categories().at( _condition.category_used_ ).str();
            ...
            case enums::DataUsed::lag:
                {
                    const auto col1 =
                        make_colname( output_.time_stamps_name(), "t1" );
    ...
    switch ( _data_used )
        {
            ...
            case enums::DataUsed::subfeatures:
                {
                    const auto number = feature_prefix_ +
                                        std::to_string( _peripheral + 1 ) +
                                        "_" + std::to_string( _input_col + 1 );

                    return "COALESCE( f_" + number + "." + quote1 + "feature_" +
                           number + quote2 + ", 0.0 )";
                }
            ...
            case enums::DataUsed::same_units_categorical:
                {
                    ...
                    const auto name2 = make_colname(
                        input_.categorical_name( _input_col ), "t2" );
            ...
            case enums::DataUsed::same_units_discrete:
                {
                    ...
                    const auto name1 = make_colname(
                        output_.discrete_name( _output_col ), "t1" );

                    const auto name2 = make_colname(
                        input_.discrete_name( _input_col ), "t2" );
            ...
            case enums::DataUsed::same_units_numerical:
                {
                    ...
                    const auto name1 = make_colname(
                        output_.numerical_name( _output_col ), "t1" );

                    const auto name2 = make_colname(
                        input_.numerical_name( _input_col ), "t2" );
