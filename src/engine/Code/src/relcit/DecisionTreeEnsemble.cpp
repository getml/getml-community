    ...
    for ( size_t i = 0; i < trees_objects.size(); ++i )
        {
            ...
            trees().push_back( decisiontrees::DecisionTree(
                impl().hyperparameters_, loss_function_, *ptr ) );
        }
    ...
    const std::vector<containers::Subfeatures> &_subfeatures )
{
    ...
    for ( size_t ix_table_used = 0;
          ...
          ++ix_table_used )
        {
            ...
            const auto
                [output_rescaled, input_rescaled, output_scaler, input_scaler] =
            ...
            const auto aggregations = make_aggregations(
                _loss_function,
                matches,
                *input_table,
                output_table,
    ...
    for ( size_t j = 0; j < num_features(); ++j )
        {
            ...
            for ( size_t i = 0; i < _population.nrows(); ++i )
                ...
                {
                    const auto p =
                        features.at( j )->at( i ) + trees().at( j ).intercept();

                    predictions[i] += p * hyperparameters().shrinkage_ *
                                      trees().at( j ).update_rate();
                }
        }
    ...
    const bool _schema_only ) const
{
    ...
    // ------------------------------------------------------------------------
    // When relcit is used as a predictor, it has no peripheral_ or
    // placeholder_.
