...
void MemoryMappedEncoding::append(
    const MemoryMappedEncoding& _other, bool _include_subencoding )
{
...
Int MemoryMappedEncoding::insert(
    const strings::String& _str, const std::optional<size_t>& _opt )
{
    ...
    if ( !_opt ) [[likely]]
    ...
    else if ( rownums()[*_opt].first != HASH_COLLISION )
        {
            ...
            assert_true(
                string_vector()[rownums()[*_opt].first - subsize_] != _str );
            auto p = std::make_pair(
                HASH_COLLISION, memmap::Vector<Int>( pool_ ).yield_impl() );
...
Int MemoryMappedEncoding::string_to_int( const strings::String& _val ) const
{
    ...
    if ( p.first != HASH_COLLISION )
        {
            return string_vector()[p.first - subsize_] == _val ? p.first
                                                               : NOT_FOUND;
        }
