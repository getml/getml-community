...
void Imputation::extract_and_add(
    ...
    containers::DataFrame* _df )
{
    const bool all_nan = std::all_of(
        _original_col.begin(), _original_col.end(), []( Float val ) {
    ...
    containers::DataFrame* _df ) const
{
    ...
    replacement_col.set_name(
        make_name( _original_col.name(), _imputation_value ) );
    ...
    _df->add_float_column(
        replacement_col, containers::DataFrame::ROLE_NUMERICAL );
...
containers::DataFrame Imputation::transform_df(
    const containers::DataFrame& _df,
    const std::string& _marker,
