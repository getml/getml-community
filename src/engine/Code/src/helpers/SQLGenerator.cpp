    ...
    for ( size_t i = 0; i < join_keys1.size(); ++i )
        {
            if ( _for_staging )
                {
                    sql << _sql_dialect_generator->edit_colname(
                        join_keys1.at( i ), _output_alias );

                    sql << " = ";

                    sql << _sql_dialect_generator->edit_colname(
                        join_keys2.at( i ), _input_alias );
            ...
            else
                {
                    ...
                    sql << _output_alias << "." << quote1
                        << _sql_dialect_generator->make_colname(
                               join_keys1.at( i ) )
                        << quote2;
...
std::string SQLGenerator::replace_non_alphanumeric( const std::string _old )
{
    ...
    const auto shorten = []( const std::string& _str ) -> std::string {
        ...
        while ( str.find( "___" ) != std::string::npos )
            {
                str = StringReplacer::replace_all( str, "___", "__" );
            }
