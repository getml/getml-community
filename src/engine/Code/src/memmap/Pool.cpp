...
size_t Pool::allocate_block(
    const size_t _block_size, const size_t _current_page )
{
...
bool Pool::current_block_can_be_extended(
    const size_t _block_size, const size_t _current_page ) const
{
    if ( _current_page == NOT_ALLOCATED )
        {
            return false;
    ...
    return std::all_of(
        pages_ + _current_page + pages_[_current_page].block_size_,
        pages_ + _current_page + _block_size,
        is_free );
}
...
void Pool::init_pages(
    const size_t _first_new_page, const size_t _last_new_page )
{
...
char* Pool::memmap( const int _fd, const size_t _length ) const
{
    ...
    auto addr =
        mmap( NULL, _length, PROT_READ | PROT_WRITE, MAP_SHARED, _fd, 0 );

    assert_msg(
        addr != MAP_FAILED,
        "_fd: " + std::to_string( _fd ) + ", _length: " +
            std::to_string( _length ) + " " + strerror( errno ) );

    if ( addr == MAP_FAILED )
        {
            throw std::runtime_error(
                "Could not map onto file: " +
...
void Pool::move_data_to_new_block(
    const size_t _old_page_num, const size_t _new_page_num )
{
    assert_true( _old_page_num < num_pages_ );
    assert_true( _new_page_num < num_pages_ );
    assert_true(
        pages_[_old_page_num].block_size_ < pages_[_new_page_num].block_size_ );
...
void Pool::remap( const size_t _num_pages )
{
    ...
    pages_ = reinterpret_cast<Page*>(
        memmap( fd_pages_, _num_pages * sizeof( Page ) ) );
}
