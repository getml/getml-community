...
class Pool
{
   ...
   public:
    Pool( const std::string &_temp_dir );
   ...
   public:
    ...
    template <class T>
    size_t allocate(
        const size_t _num_elements, const size_t _current_page = NOT_ALLOCATED )
    {
   ...
   private:
    ...
    std::pair<std::string, int> create_file(
        const std::string &_temp_dir ) const;

    /// Whether the current block can just be extended
    bool current_block_can_be_extended(
        const size_t _num_pages, const size_t _current_page ) const;
