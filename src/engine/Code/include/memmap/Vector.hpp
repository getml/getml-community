...
class Vector
{
   ...
   public:
    Vector( const std::shared_ptr<Pool> &_pool )
        : impl_( VectorImpl<T>(
              VectorImpl<T>::NOT_ALLOCATED,
              _pool ? _pool.get() : nullptr,
              0 ) ),
          pool_( _pool )
    {
        assert_true( pool_ );
        const auto capacity = sizeof( T ) < pool().page_size()
