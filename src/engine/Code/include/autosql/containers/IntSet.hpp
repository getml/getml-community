...
{
   public:
    IntSet( const SQLNET_INT _maximum_value )
    ...
    std::vector<SQLNET_INT>::const_iterator begin() const
    {
    ...
    void clear()
    {
        for ( SQLNET_INT i : unique_integers_ )
            {
    ...
    std::vector<SQLNET_INT>::const_iterator end() const
    {
        return unique_integers_.cend();
    }

    /// Adds an integer to unique_integers_, if it is not already included
    void insert( const SQLNET_INT _val )
    {
        assert( _val >= 0 );
        assert( _val < static_cast<SQLNET_INT>( already_included_.size() ) );
    ...
    /// Trivial getter
    const SQLNET_INT maximum_value() const { return maximum_value_; }

    /// Whether the IntSet is empty
    size_t size() const { return unique_integers_.size(); }

    /// Trivial getter
    const std::vector<SQLNET_INT>& unique_integers() const
    {
   ...
   private:
    ...
    const SQLNET_INT maximum_value_;

    /// Contains all integers that have been included
    std::vector<SQLNET_INT> unique_integers_;
};
