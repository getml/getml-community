...
class AggregationBase
{
   public:
    ...
    virtual void activate_all(
        ...
        SQLNET_SAMPLE_ITERATOR _sample_container_end ) = 0;
    ...
    virtual void deactivate_samples_with_null_values(
        SQLNET_SAMPLE_ITERATOR _sample_container_begin,
        SQLNET_SAMPLE_ITERATOR _null_values_separator ) = 0;
    ...
    virtual SQLNET_SAMPLES::iterator separate_null_values(
        SQLNET_SAMPLES &_samples ) = 0;

    /// Separates the pointers to samples for which the value to be aggregated
    /// is NULL
    virtual SQLNET_SAMPLE_CONTAINER::iterator separate_null_values(
        SQLNET_SAMPLE_CONTAINER &_samples ) = 0;
    ...
    virtual void set_value_to_be_aggregated(
        ...
        const SQLNET_INT _ix_column_used ) = 0;

    /// Trivial setter
    virtual void set_value_to_be_aggregated(
        const containers::ColumnView<
            SQLNET_FLOAT,
            std::map<SQLNET_INT, SQLNET_INT>> &_value_to_be_aggregated ) = 0;
    ...
    virtual void sort_samples(
        ...
        SQLNET_SAMPLES::iterator _samples_end ) = 0;
    ...
    virtual containers::Matrix<SQLNET_FLOAT> &yhat() = 0;
};
