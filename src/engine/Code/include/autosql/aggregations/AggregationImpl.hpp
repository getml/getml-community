...
{
    AggregationImpl( SQLNET_INT _sample_size )
        : count_( std::vector<SQLNET_FLOAT>( _sample_size ) ),
          count_committed_( std::vector<SQLNET_FLOAT>( _sample_size ) ),
          sample_ptr_( std::vector<Sample*>( _sample_size ) ),
          sample_ptr_committed_( std::vector<Sample*>( _sample_size ) ),
          sum_( std::vector<SQLNET_FLOAT>( _sample_size ) ),
          sum_committed_( std::vector<SQLNET_FLOAT>( _sample_size ) ),
          sum_cubed_( std::vector<SQLNET_FLOAT>( _sample_size ) ),
          sum_cubed_committed_( std::vector<SQLNET_FLOAT>( _sample_size ) ),
          sum_squared_( std::vector<SQLNET_FLOAT>( _sample_size ) ),
          sum_squared_committed_( std::vector<SQLNET_FLOAT>( _sample_size ) ),
          updates_current_( containers::IntSet( _sample_size ) ),
          updates_stored_( containers::IntSet( _sample_size ) ),
          yhat_( containers::Matrix<SQLNET_FLOAT>( _sample_size, 1 ) ),
          yhat_committed_( std::vector<SQLNET_FLOAT>( _sample_size ) ),
          yhat_stored_( std::vector<SQLNET_FLOAT>( _sample_size ) )
    {
    }
    ...
    /// committed
    std::vector<SQLNET_FLOAT> count_committed_;
    ...
    /// from yhat
    containers::ColumnView<SQLNET_FLOAT, std::map<SQLNET_INT, SQLNET_INT>>
        value_to_be_aggregated_;

    /// Value to be aggregated to be used for aggregations that can be
    /// categorical
    containers::ColumnView<SQLNET_INT, std::map<SQLNET_INT, SQLNET_INT>>
    ...
    containers::ColumnView<SQLNET_FLOAT, std::vector<SQLNET_INT>>
