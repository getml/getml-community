...
void test17_nan_values_discrete()
{
    ...
    std::cout << std::endl
              << "Test 17 (NAN values, discrete): " << std::endl
              << std::endl;
    ...
    const auto population_json = load_json( "../../tests/autosql/test17/schema.json" );
    ...
    const auto hyperparameters_json =
        load_json( "../../tests/autosql/test17/hyperparameters.json" );
    ...
    model.save( "../../tests/autosql/test17/Model.json" );

    // ------------------------------------------------------------------------
    // Express as SQL code.

    std::ofstream sql( "../../tests/autosql/test17/Model.sql" );
    sql << model.to_sql();
    sql.close();
