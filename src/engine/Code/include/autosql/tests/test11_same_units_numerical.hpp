...
void test11_same_units_numerical()
{
    ...
    for ( size_t i = 0; i < peripheral_df.nrows(); ++i )
        {
            ...
            if ( peripheral_df.time_stamp( i ) <=
                 ...
                 numerical_population[jk] - peripheral_df.numerical( i, 0 ) <
                     250.0 )
                {
    ...
    const auto population_json = load_json( "../../tests/autosql/test11/schema.json" );
    ...
    model.save( "../../tests/autosql/test11/Model.json" );

    // ------------------------------------------------------------------------
    // Express as SQL code.

    std::ofstream sql( "../../tests/autosql/test11/Model.sql" );
    sql << model.to_sql();
    sql.close();
