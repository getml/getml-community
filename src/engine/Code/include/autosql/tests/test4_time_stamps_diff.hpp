...
void test4_time_stamps_diff()
{
    // ------------------------------------------------------------------------

    std::cout << std::endl
              << "Test 4 (difference between time stamps): " << std::endl
              << std::endl;
    ...
    const auto population_json = load_json( "../../tests/autosql/test4/schema.json" );
    ...
    const auto hyperparameters_json =
        load_json( "../../tests/autosql/test4/hyperparameters.json" );
    ...
    model.save( "../../tests/autosql/test4/Model.json" );

    // ------------------------------------------------------------------------
    // Express as SQL code.

    std::ofstream sql( "../../tests/autosql/test4/Model.sql" );
    sql << model.to_sql();
    sql.close();
