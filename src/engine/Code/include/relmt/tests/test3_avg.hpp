...
void test3_avg( const std::filesystem::path _test_path )
{
    // ------------------------------------------------------------------------

    auto test3_path = _test_path;

    test3_path.append( "relmt" ).append( "test3" );

    // ------------------------------------------------------------------------

    std::cout << "Test 3 | AVG aggregation\t\t\t\t";
    ...
    const auto numerical_peripheral_col = relmt::containers::Column<double>(
    ...
    const auto time_stamps_peripheral_col = relmt::containers::Column<double>(
        time_stamps_peripheral.data(),
        "time_stamp",
        time_stamps_peripheral.size() );

    const auto peripheral_df = relmt::containers::DataFrame(
        {},
        {},
    ...
    for ( size_t i = 0; i < peripheral_df.nrows(); ++i )
        {
            ...
            if ( peripheral_df.time_stamp( i ) <=
                ...
                {
                    assert_true( counts.at( jk ) > 0.0 );

                    targets_population.at( jk ) +=
                        peripheral_df.numerical( i, 0 ) / counts.at( jk );
                }
        }
    ...
    auto hyperparameters_path = test3_path;
    ...
    const auto hyperparameters =
        std::make_shared<const relmt::Hyperparameters>( *hyperparameters_json );
