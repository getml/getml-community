...
void test18_upper_time_stamps()
{
    ...
    const auto categorical_peripheral_mat =
        relboost::containers::Matrix<std::int32_t>( {}, nullptr, 250000 );
    ...
    for ( size_t i = 0; i < upper_time_stamps_peripheral.size(); ++i )
        {
            upper_time_stamps_peripheral[i] = time_stamps_peripheral[i] + 50.0;
        }

    const auto upper_time_stamps_peripheral_mat =
        relboost::containers::Matrix<double>(
            {"upper_time_stamp"},
            upper_time_stamps_peripheral.data(),
            upper_time_stamps_peripheral.size() );
    ...
    const auto categorical_population_mat =
        relboost::containers::Matrix<std::int32_t>( {}, nullptr, 500 );
    ...
    for ( size_t i = 0; i < peripheral_df.nrows(); ++i )
        {
            ...
            if ( peripheral_df.time_stamps( i ) <=
                 ...
                 peripheral_df.upper_time_stamps( i ) >
                     time_stamps_population_mat( jk, 0 ) &&
    ...
    const auto population_json = load_json( "../../tests/test18/schema.json" );
    ...
    const auto hyperparameters_json =
        load_json( "../../tests/test18/hyperparameters.json" );
    ...
    model.save( "../../tests/test18/Model.json" );

    // ------------------------------------------------------------------------
    // Express as SQL code.

    std::ofstream sql( "../../tests/test18/Model.sql" );
    sql << model.to_sql();
    sql.close();
    ...
    for ( size_t i = 0; i < predictions.size(); ++i )
        {
            ...
            assert(
                std::abs( population_df.target_[i] - predictions[i] ) < 10.0 );
        }
