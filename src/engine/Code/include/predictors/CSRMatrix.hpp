...
template <typename T>
CSRMatrix<T>::CSRMatrix( const CFloatColumn& _col )
{
    assert( _col );
...
template <typename T>
CSRMatrix<T>::CSRMatrix( const CIntColumn& _col, const size_t _n_unique )
{
    assert( _col );
    ...
    data_ = std::vector<T>( indices_.size(), 1.0 );
...
template <typename T>
void CSRMatrix<T>::add( const CFloatColumn& _col )
{
    // -------------------------------------------------------------------------
    ...
    auto data_temp = std::vector<T>( data_.size() + _col->size() );
    ...
    for ( size_t i = 0; i < nrows(); ++i )
        {
            ...
            data_temp[indptr_[i + 1] + i] = static_cast<T>( ( *_col )[i] );
        }
...
template <typename T>
void CSRMatrix<T>::add( const CIntColumn& _col, const size_t _n_unique )
{
    // -------------------------------------------------------------------------
    ...
    auto data_temp = std::vector<T>( data_.size() + num_non_negative );
