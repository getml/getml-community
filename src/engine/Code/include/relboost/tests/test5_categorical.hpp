...
void test5_categorical()
{
    ...
    std::cout << std::endl
              << "Test 5 (SUM aggregation, categorical): " << std::endl
              << std::endl;
    ...
    const auto numerical_peripheral_mat =
        relboost::containers::Matrix<double>( {}, nullptr, 250000 );
    ...
    const auto categorical_population_mat =
        relboost::containers::Matrix<std::int32_t>(
            {"column_01"}, categorical_population.data(), 500 );
    ...
    for ( size_t i = 0; i < peripheral_df.nrows(); ++i )
        {
            ...
            if ( peripheral_df.time_stamps( i ) <=
                 ...
                 peripheral_df.categorical_( i, 0 ) == 3 )
                {
    ...
    const auto population_json = load_json( "../../tests/test5/schema.json" );
    ...
    const auto hyperparameters_json =
        load_json( "../../tests/test5/hyperparameters.json" );
    ...
    model.save( "../../tests/test5/Model.json" );

    // ------------------------------------------------------------------------
    // Express as SQL code.

    std::ofstream sql( "../../tests/test5/Model.sql" );
    sql << model.to_sql();
    sql.close();
