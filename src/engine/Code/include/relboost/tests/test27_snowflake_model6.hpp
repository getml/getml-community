...
void test27_snowflake_model6()
{
    // ------------------------------------------------------------------------

    std::cout << std::endl
              << "Test 27 (snowflake model, SUM of AVG with categorical): "
              << std::endl
              << std::endl;
    ...
    auto counts = std::vector<double>( peripheral1_df.nrows() );

    for ( size_t i = 0; i < peripheral2_df.nrows(); ++i )
        for ( size_t j = 0; j < peripheral1_df.nrows(); ++j )
            {
                if ( join_key2_peripheral2[i] == join_key2_peripheral1[j] &&
                     time_stamp2_peripheral2[i] <= time_stamp2_peripheral1[j] )
                    {
                        ++counts[j];
                    }
            }
    ...
    const auto population_json =
        load_json( "../../tests/relboost/test27/schema.json" );
    ...
    const auto hyperparameters_json =
        load_json( "../../tests/relboost/test27/hyperparameters.json" );
    ...
    model.save( "../../tests/relboost/test27/Model.json" );

    // ------------------------------------------------------------------------
    // Express as SQL code.

    std::ofstream sql( "../../tests/relboost/test27/Model.sql" );
    sql << model.to_sql();
    sql.close();
