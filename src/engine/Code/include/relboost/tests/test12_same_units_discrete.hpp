...
void test12_same_units_discrete()
{
    // ------------------------------------------------------------------------

    std::cout << std::endl
              << "Test 12 (same units discrete): " << std::endl
              << std::endl;
    ...
    for ( size_t i = 0; i < peripheral_df.nrows(); ++i )
        {
            ...
            if ( peripheral_df.time_stamp( i ) <=
                 ...
                 discrete_population[jk] - peripheral_df.discrete( i, 0 ) <
                     50.0 )
                {
    ...
    const auto population_json = load_json( "../../tests/relboost/test12/schema.json" );
    ...
    const auto hyperparameters_json =
        load_json( "../../tests/relboost/test12/hyperparameters.json" );
    ...
    model.save( "../../tests/relboost/test12/Model.json" );

    // ------------------------------------------------------------------------
    // Express as SQL code.

    std::ofstream sql( "../../tests/relboost/test12/Model.sql" );
    sql << model.to_sql();
    sql.close();
