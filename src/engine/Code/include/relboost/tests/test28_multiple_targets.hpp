...
void test28_multiple_targets()
{
    ...
    auto targets_population2 = std::vector<double>( 500 );

    const auto target_population_col = relboost::containers::Column<double>(
        targets_population.data(), "target", targets_population.size() );

    const auto target_population_col2 = relboost::containers::Column<double>(
        targets_population2.data(), "target2", targets_population2.size() );
    ...
    const auto population_df = relboost::containers::DataFrame(
        ...
        {target_population_col, target_population_col2},
    ...
    for ( size_t i = 0; i < peripheral_df.nrows(); ++i )
        {
            ...
            if ( peripheral_df.time_stamp( i ) <=
                     time_stamps_population_col[jk] &&
                 peripheral_df.numerical( i, 0 ) < 350.0 )
                {
                    targets_population2[jk]++;
                }
        }
    ...
    auto model2 = relboost::ensemble::DecisionTreeEnsemble(
        encoding, hyperparameters2, peripheral, population );
    ...
    assert_true( predictions2.size() == population_df.nrows() );

    for ( size_t i = 0; i < predictions2.size(); ++i )
        {
            /*  std::cout << "target: " << population_df.target( i, 1 )
                        << ", prediction: " << predictions2[i] << std::endl;*/
