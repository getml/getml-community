...
void test9_discrete()
{
    ...
    std::cout << std::endl
              << "Test 9 (discrete columns): " << std::endl
              << std::endl;
    ...
    const auto discrete_peripheral_mat = relboost::containers::Matrix<double>(
        {"column_01"}, discrete_peripheral.data(), discrete_peripheral.size() );
    ...
    for ( size_t i = 0; i < peripheral_df.nrows(); ++i )
        {
            ...
            if ( peripheral_df.time_stamps( i ) <=
                 ...
                 peripheral_df.discrete_( i, 0 ) < 250.0 )
                {
    ...
    const auto population_json = load_json( "../../tests/test9/schema.json" );
    ...
    const auto hyperparameters_json =
        load_json( "../../tests/test9/hyperparameters.json" );
    ...
    model.save( "../../tests/test9/Model.json" );

    // ------------------------------------------------------------------------
    // Express as SQL code.

    std::ofstream sql( "../../tests/test9/Model.sql" );
    sql << model.to_sql();
    sql.close();
