...
void test9_discrete()
{
    ...
    for ( size_t i = 0; i < peripheral_df.nrows(); ++i )
        {
            ...
            if ( peripheral_df.time_stamp( i ) <=
                 ...
                 peripheral_df.discrete( i, 0 ) < 250.0 )
                {
    ...
    const auto population_json = load_json( "../../tests/relboost/test9/schema.json" );
    ...
    const auto hyperparameters_json =
        load_json( "../../tests/relboost/test9/hyperparameters.json" );
    ...
    model.save( "../../tests/relboost/test9/Model.json" );

    // ------------------------------------------------------------------------
    // Express as SQL code.

    std::ofstream sql( "../../tests/relboost/test9/Model.sql" );
    sql << model.to_sql();
    sql.close();
