...
void test20_saving_and_loading( std::filesystem::path _test_path )
{
    ...
    std::cout << "Test 20 | saving and loading\t\t\t\t";
    ...
    auto model2 = relboost::ensemble::DecisionTreeEnsemble( *model_json );

    model2.save( tmp_filename_json_2 );

    const auto model2_json = load_json( tmp_filename_json_2 );

    auto model3 = relboost::ensemble::DecisionTreeEnsemble( *model2_json );
    ...
    const auto predictions2 =
        model2.predict( population_df, { peripheral_df } );

    const auto predictions3 =
        model3.predict( population_df, { peripheral_df } );
    ...
    for ( size_t i = 0; i < predictions.size(); ++i )
        {
            ...
            assert_true( std::abs( predictions[i] - predictions2[i] ) < 1e-07 );

            assert_true( std::abs( predictions[i] - predictions3[i] ) < 1e-07 );
        }
