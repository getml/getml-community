...
class ValueContainerCreator
{
   private:
    ...
    typedef std::conditional<
        ...
        ValueContainerType,
        void>::type SameUnitNumericalType;
    ...
    const containers::Subfeatures &_subfeatures )
{
    ...
    if constexpr ( data_used_ == enums::DataUsed::x_perip_numerical )
        {
            const auto col =
                NumericalColumn( _peripheral.numerical_col( ix_column_used ) );
    ...
    if constexpr ( data_used_ == enums::DataUsed::x_perip_discrete )
        {
            const auto col =
                NumericalColumn( _peripheral.discrete_col( ix_column_used ) );
    ...
    if constexpr ( data_used_ == enums::DataUsed::time_stamps_diff )
        {
            const auto numerical =
                NumericalColumn( _peripheral.time_stamp_col() );

            const auto comparison =
                ComparisonColumn( _population.time_stamp_col() );
    ...
    if constexpr (
        ...
        {
            static_assert(
                std::is_same<ValueContainerType, SameUnitNumericalType>(),
                "Does not return!" );

            return create_same_unit_numerical(
    ...
    if constexpr (
        ...
        {
            static_assert(
                std::is_same<ValueContainerType, SameUnitDiscreteType>(),
                "Does not return!" );

            return create_same_unit_discrete(
    ...
    if constexpr ( data_used_ == enums::DataUsed::x_perip_categorical )
        {
            const auto col = CategoricalColumn(
    ...
    if constexpr ( data_used_ == enums::DataUsed::x_subfeature )
        {
            const auto col =
                NumericalColumn( _subfeatures.at( ix_column_used ) );
...
template <enums::DataUsed data_used_, bool is_population_>
typename ValueContainerCreator<
    data_used_,
    ...
    if constexpr ( std::is_same<ValueContainerType, SameUnitNumericalType>() )
        {
            ...
            if ( data_used2 == enums::DataUsed::x_popul_numerical )
                {
                    assert_true(
                        _population.num_numericals() > ix_column_used2 );
