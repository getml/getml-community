#ifndef MULTIREL_CONTAINERS_SUMMARIZER_HPP_
#define MULTIREL_CONTAINERS_SUMMARIZER_HPP_
...
class Summarizer
{
   public:
    ...
    /// a set of targets.
    static std::vector<std::vector<Float>> calculate_feature_correlations(
...
#ifdef MULTIREL_PARALLEL

    /// Identifies the global minimum and the global maximum
    template <typename T>
    static void reduce_min_max(
        multithreading::Communicator& _comm, T& _min, T& _max );

#endif  // MULTIREL_PARALLEL
   ...
   private:
    ...
    static std::vector<Float> share_nan( const Matrix<Float>& _mat );

    /// Finds the share of nan (for Int)
    static std::vector<Float> share_nan( const Matrix<Int>& _mat );
...
template <typename T>
void Summarizer::reduce_min_max(
    multithreading::Communicator& _comm, T& _min, T& _max )
{
    T global_min = 0;
    ...
    MULTIREL_PARALLEL_LIB::all_reduce(
        _comm,                // comm
        _min,                 // in_value
        global_min,           // out_value
        MULTIREL_MIN_OP<T>()  // op
    );
...
#endif  // MULTIREL_CONTAINERS_SUMMARIZER_HPP_
