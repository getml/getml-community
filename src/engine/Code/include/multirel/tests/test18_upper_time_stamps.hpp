...
void test18_upper_time_stamps( std::filesystem::path _test_path )
{
    // ------------------------------------------------------------------------
    ...
    _test_path.append( "multirel" ).append( "test18" ).append( "schema.json" );
    ...
    std::ofstream sql( _test_path.replace_filename( "Model.sql" ) );
    sql << model.to_sql();
    sql.close();
