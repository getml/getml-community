...
class SparkSQLGenerator : public SQLDialectGenerator
{
   ...
   public:
    ...
    /// code.
    std::string edit_colname(
        const std::string& _raw_name, const std::string& _alias ) const final;

    /// Generates the SQL code necessary for joining the mapping tables onto the
    /// staged table.
    std::string join_mapping(
    ...
    std::string make_mapping_table_header(
        const std::string& _name, const bool _key_is_num ) const final;
   ...
   private:
    ...
    std::string avg_time_between_aggregation(
    ...
    std::string count_above_below_mean_aggregation(
    ...
    std::vector<std::string> make_staging_columns(
        const bool& _include_targets, const Schema& _schema ) const;
    ...
    std::string make_staging_table(
        const bool& _include_targets, const Schema& _schema ) const;

    /// Generates the code for a linear trend.
    std::string make_trend_aggregation(
