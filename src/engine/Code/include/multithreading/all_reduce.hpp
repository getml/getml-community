    ...
    const Operator _op )
{
    ...
    // ---------------------------------------------------------
    // Apply operator to global data (the first thread to
    // reach the communicator resizes the global data and
    // simply copies the _in_values).
    ...
    if ( ( _comm.num_threads_left() )-- == _comm.num_threads() )
        {
            ...
            std::copy(
                _in_values, _in_values + _count, _comm.global_data<T>() );
        }
    else
        {
            std::transform(
                _comm.global_data_const<T>(),
                _comm.global_data_const<T>() + _count,
                _in_values,
                _comm.global_data<T>(),
                _op );
