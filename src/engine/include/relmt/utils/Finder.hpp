#ifndef RELMT_UTILS_FINDER_HPP_
#define RELMT_UTILS_FINDER_HPP_
...
struct Finder<enums::DataUsed::discrete_input> {
  static std::vector<containers::Match>::iterator next_split(
      const Float _cv, const size_t _num_column,
      const containers::DataFrame& _input,
      const std::vector<containers::Match>::iterator _begin,
      const std::vector<containers::Match>::iterator _end) {
    const auto smaller_than_cv = [&_input, _num_column,
...
struct Finder<enums::DataUsed::discrete_output> {
  static std::vector<containers::Match>::iterator next_split(
      ...
      const std::vector<containers::Match>::iterator _begin,
      const std::vector<containers::Match>::iterator _end) {
    const auto smaller_than_cv = [&_output, _num_column,
                                  _cv](containers::Match m) {
...
struct Finder<enums::DataUsed::same_units_discrete> {
  static std::vector<containers::Match>::iterator next_split(
      const Float _cv, const size_t _input_col, const size_t _output_col,
      const containers::DataFrame& _input,
      const containers::DataFrameView& _output,
      const std::vector<containers::Match>::iterator _begin,
      const std::vector<containers::Match>::iterator _end) {
    auto smaller_than_cv = [&_input, &_output, _input_col, _output_col,
                            _cv](containers::Match m) {
      assert_true(m.ix_input < _input.nrows());
      assert_true(m.ix_output < _output.nrows());
