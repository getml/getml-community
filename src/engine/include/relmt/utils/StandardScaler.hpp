...
class StandardScaler {
 public:
  ...
  StandardScaler(const Poco::JSON::Object& _obj) {
    means_ = JSON::array_to_vector<Float>(JSON::get_array(_obj, "means_"));

    inverse_stddev_ =
        JSON::array_to_vector<Float>(JSON::get_array(_obj, "inverse_stddev_"));
  }
 ...
 public:
  ...
  Poco::JSON::Object::Ptr to_json_obj() const {
    ...
    obj->set("inverse_stddev_", JSON::vector_to_array_ptr(inverse_stddev_));
    return obj;
  }
 ...
 private:
  ...
  Float apply_operator(const ColumnType& _col, const OpType& _op,
  ...
  containers::Rescaled calc_rescaled(
      ...
      const std::vector<Float>& _inverse_stddev, const DataFrameType& _df,
      const std::optional<containers::Subfeatures>& _subfeatures,
  ...
  template <class DataFrameType>
  std::pair<std::shared_ptr<const containers::Rescaled::MapType>,
            std::vector<size_t>>
  calc_rows_map(const DataFrameType& _df,
                const std::vector<containers::Match>::iterator _begin,
                const std::vector<containers::Match>::iterator _end,
                const std::shared_ptr<containers::Rescaled::MapType> _rows_map =
                    nullptr) const;
  ...
  Float calc_mean(const ColumnType& _col,
                  const std::vector<size_t>& _unique_indices) const;
  ...
  std::vector<Float> calc_inverse_stddev(
      const std::vector<Float>& _means, const DataFrameType& _df,
      const std::optional<containers::Subfeatures>& _subfeatures,
      const std::vector<size_t>& _unique_indices) const;

  /// Calculates the variance of a column.
  template <class ColumnType>
  Float calc_variance(const ColumnType& _col, const Float _mean,
                      const std::vector<size_t>& _unique_indices) const;
...
Float StandardScaler::apply_operator(
    const ColumnType& _col, const OpType& _op,
    const std::vector<size_t>& _unique_indices) const {
...
std::vector<Float> StandardScaler::calc_means(
  ...
  for (size_t j = 0; j < _df.num_discretes(); ++j) {
    means.push_back(calc_mean(_df.discrete_col(j), _unique_indices));
  }

  for (size_t j = 0; j < _df.num_numericals(); ++j) {
    means.push_back(calc_mean(_df.numerical_col(j), _unique_indices));
  }

  if (_subfeatures) {
    for (size_t j = 0; j < _subfeatures->size(); ++j) {
      means.push_back(calc_mean(_subfeatures->at(j), _unique_indices));
    }
  }
...
std::vector<Float> StandardScaler::calc_inverse_stddev(
  ...
  for (size_t j = 0; j < _df.num_discretes(); ++i, ++j) {
    ...
    inverse_stddev.at(i) =
        calc_variance(_df.discrete_col(j), _means.at(i), _unique_indices);
  }

  for (size_t j = 0; j < _df.num_numericals(); ++i, ++j) {
    assert_true(i < _means.size());
    inverse_stddev.at(i) =
        calc_variance(_df.numerical_col(j), _means.at(i), _unique_indices);
  }

  if (_subfeatures) {
    for (size_t j = 0; j < _subfeatures->size(); ++i, ++j) {
      inverse_stddev.at(i) =
          calc_variance(_subfeatures->at(j), _means.at(i), _unique_indices);
    }
  }
  ...
  std::transform(inverse_stddev.begin(), inverse_stddev.end(),
                 inverse_stddev.begin(), sqrt_invert);
...
containers::Rescaled StandardScaler::calc_rescaled(
    const std::vector<Float>& _means, const std::vector<Float>& _inverse_stddev,
    const DataFrameType& _df,
    const std::optional<containers::Subfeatures>& _subfeatures,
    const std::shared_ptr<const containers::Rescaled::MapType>& _rows_map,
    const std::vector<size_t>& _unique_indices) const {
  ...
  for (size_t j = 0; j < _df.num_discretes(); ++j, ++k) {
    ...
    for (size_t i = 0; i < map_size; ++i) {
      ...
      data->at(i * ncols + k) = rescale(_df.discrete(ix, j), k);
    }
  }
  ...
  for (size_t j = 0; j < _df.num_numericals(); ++j, ++k) {
    ...
    for (size_t i = 0; i < map_size; ++i) {
      ...
      data->at(i * ncols + k) = rescale(_df.numerical(ix, j), k);
    }
  }
  ...
  if (_subfeatures) {
    for (size_t j = 0; j < _subfeatures->size(); ++j, ++k) {
      ...
      for (size_t i = 0; i < map_size; ++i) {
        ...
        data->at(i * ncols + k) = rescale(_subfeatures->at(j)[ix], k);
      }
    }
  }
...
StandardScaler::calc_rows_map(
  ...
  return std::make_pair(rows_map, std::vector<size_t>(unique_indices.begin(),
                                                      unique_indices.end()));
}
...
Float StandardScaler::calc_variance(
    const ColumnType& _col, const Float _mean,
    const std::vector<size_t>& _unique_indices) const {
...
containers::Rescaled StandardScaler::fit_transform(
  ...
  return calc_rescaled(means_, inverse_stddev_, _df, _subfeatures, rows_map,
                       unique_indices);
}
