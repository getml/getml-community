#ifndef RELMT_UTILS_IMPORTANCEMAKER_HPP_
#define RELMT_UTILS_IMPORTANCEMAKER_HPP_
...
class ImportanceMaker {
 ...
 public:
  ...
  void add(const helpers::Schema& _input, const helpers::Schema& _output,
           const enums::DataUsed _data_used, const size_t _column,
           const size_t _column_input, const Float _value);

 public:
  /// Adds all of the colnames with importance 0.0.
  void fill_zeros(const helpers::Schema& _pl, const std::string& _tname,
  ...
  void merge(const std::map<helpers::ColumnDescription, Float>& _importances) {
 ...
 private:
  ...
  void add_to_importances(const helpers::ColumnDescription& _desc,
                          const Float _value) {
...
#endif  // RELMT_UTILS_IMPORTANCEMAKER_HPP_
