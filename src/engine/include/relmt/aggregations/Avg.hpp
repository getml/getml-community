#ifndef RELMT_AGGREGATIONS_AVG_HPP_
#define RELMT_AGGREGATIONS_AVG_HPP_
...
#include "relmt/aggregations/IntermediateAggregationImpl.hpp"
...
class Avg : public lossfunctions::LossFunction {
 ...
 public:
  ...
  Avg(const std::shared_ptr<lossfunctions::LossFunction>& _child,
      ...
      const std::optional<containers::Rescaled>& _input_rescaled = std::nullopt,
      const std::optional<containers::Rescaled>& _output_rescaled =
          std::nullopt)
      : child_(_child),
        comm_(_comm),
        ...
        nrows_(_output.nrows()),
        num_samples_1_(0.0),
        num_samples_2_(0.0),
        output_indices_(_output.indices()),
        impl_(AggregationImpl(_child.get(), &eta1_, &eta1_old_, &eta2_,
                              &eta2_old_, &indices_, &indices_current_,
                              _input_rescaled, _output_rescaled)) {
    resize(nrows(), ncols());
  ...
  Avg(const std::shared_ptr<AggregationIndex>& _agg_index,
      ...
      const containers::DataFrameView& _output,
      multithreading::Communicator* _comm)
      : Avg(_child, std::vector<containers::Match>(0), _input, _output, _comm) {
 ...
 public:
  ...
  void resize(size_t _nrows, size_t _ncols) final;
 ...
 public:
  ...
  std::pair<Float, containers::Weights> calc_pair(
      ...
      const std::vector<Float>& _eta2,
      const std::vector<Float>& _eta2_old) final {
    return intermediate_agg().calc_pair(
        true, _agg, _revert, _update, _old_weights, _indices, _indices_current,
  ...
  //  const containers::DataFrame& input() { return impl_.input(); }
  ...
  // const containers::DataFrameView& output() { return impl_.output(); }
  ...
  void reduce_predictions(
    ...
    *_predictions = intermediate_agg().reduce_predictions(true, **_predictions);
 ...
 private:
  ...
  void calc_diff(const std::vector<Float>& _old_weights,
                 const std::vector<containers::Match>::iterator _split_begin,
                 const std::vector<containers::Match>::iterator _split_end);

  /// Calculates the new yhat assuming that this is the
  /// lowest aggregation.
  void calc_yhat(const std::vector<Float>& _old_weight,
                 const containers::Weights& _new_weights);
...
#endif  // RELMT_AGGREGATIONS_AVG_HPP_
