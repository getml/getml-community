#ifndef RELMT_AGGREGATIONS_SUM_HPP_
#define RELMT_AGGREGATIONS_SUM_HPP_
...
class Sum : public lossfunctions::LossFunction {
 ...
 public:
  ...
  std::pair<Float, containers::Weights> calc_pair(
    ...
    return intermediate_agg().calc_pair(
        false, _agg, _revert, _update, _old_weights, _indices, _indices_current,
 ...
 private:
  ...
  void calc_diff(const enums::Revert _revert,
                 const std::vector<containers::Match>::iterator _split_begin,
                 const std::vector<containers::Match>::iterator _split_end);
 ...
 private:
  ...
  void calc_yhat(const std::vector<Float> _old_weights,
                 const containers::Weights& _new_weights) {
    child_->calc_yhat(enums::Aggregation::sum, _old_weights, _new_weights,
...
#endif  // RELMT_AGGREGATIONS_SUM_HPP_
