#ifndef RELMT_CONTAINERS_SPLIT_HPP_
#define RELMT_CONTAINERS_SPLIT_HPP_
...
struct Split {
  ...
  Split(const std::shared_ptr<const std::vector<Int>>& _categories_used,
        const std::vector<Int>::const_iterator _categories_used_begin,
        const std::vector<Int>::const_iterator _categories_used_end,
        const size_t _column, const enums::DataUsed _data_used)
    ...
    assert_true(_data_used == enums::DataUsed::categorical_input ||
  ...
  Split(const size_t _column, const Float _critical_value,
      ...
      : categories_used_(std::make_shared<std::vector<Int>>(0)),
        ...
        data_used_(_data_used) {
    assert_true(_data_used == enums::DataUsed::discrete_input ||
    ...
    assert_true(_data_used != enums::DataUsed::time_stamps_window ||
                _column == 0);
  }
  ...
  Split(const size_t _column, const size_t _column_input,
        const Float _critical_value, const enums::DataUsed _data_used)
      : categories_used_(std::make_shared<std::vector<Int>>(0)),
        categories_used_begin_(categories_used_->cbegin()),
        categories_used_end_(categories_used_->cend()),
        column_(_column),
        ...
        critical_value_(_critical_value),
        data_used_(_data_used) {
    assert_true(_data_used == enums::DataUsed::same_units_discrete ||
  ...
  Split deep_copy() const {
    auto sorted = std::make_shared<std::vector<Int>>(categories_used_begin_,
                                                     categories_used_end_);

    std::sort(sorted->begin(), sorted->end());

    return Split(sorted, column_, column_input_, critical_value_, data_used_);
  }
...
#endif  // RELMT_CONTAINERS_SPLIT_HPP_
