#ifndef RELMT_CONTAINERS_CATEGORYINDEX_HPP_
#define RELMT_CONTAINERS_CATEGORYINDEX_HPP_
...
#include "relmt/containers/Match.hpp"
...
class CategoryIndex {
 public:
  CategoryIndex(const std::vector<containers::Match>::iterator _begin,
 ...
 public:
  ...
  void build_indptr(const DataFrameType& _df, const size_t _num_column,
 ...
 public:
  ...
  std::vector<containers::Match>::iterator begin(const Int _category) const {
    ...
    assert_true(_category - minimum_ + 1 < static_cast<Int>(indptr_.size()));
 ...
 private:
  ...
  template <
      ...
      typename std::enable_if<_data_used == enums::DataUsed::categorical_input,
                              int>::type = 0>
  ...
  template <
      enums::DataUsed _data_used,
      typename std::enable_if<_data_used == enums::DataUsed::categorical_output,
                              int>::type = 0>
  static size_t get_num_row(containers::Match _m) {
    return _m.ix_output;
  }
...
template <enums::DataUsed _data_used, typename DataFrameType>
void CategoryIndex::build_indptr(const DataFrameType& _df,
  ...
  if (indptr_.size() > 0) {
    for (auto cat = _critical_values.front(); cat <= _critical_values.back();
         ++cat) {
      for (auto it = begin(cat); it < end(cat); ++it) {
        const auto num_row = get_num_row<_data_used>(*it);

        assert_true(_df.categorical(num_row, _num_column) == cat);
      }
    }
  }
...
#endif  // RELMT_CONTAINERS_CATEGORYINDEX_HPP_
