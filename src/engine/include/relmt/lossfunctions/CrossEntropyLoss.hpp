#ifndef RELMT_LOSSFUNCTIONS_CROSSENTROPYLOSS_HPP_
#define RELMT_LOSSFUNCTIONS_CROSSENTROPYLOSS_HPP_
...
class CrossEntropyLoss : public LossFunction {
 ...
 public:
  CrossEntropyLoss(
      ...
      : comm_(nullptr),
        ...
        impl_(LossFunctionImpl(g_, h_, hyperparameters_, nrows_, sample_index_,
                               sample_weights_, sum_g_, sum_h_,
                               sum_h_yhat_committed_, sum_sample_weights_,
                               targets_)) {
    assert_true(std::abs(logistic_function(inverse_logistic_function(0.0001)) -
                         0.0001) < 1e-7);

    assert_true(std::abs(logistic_function(inverse_logistic_function(0.001)) -
                         0.001) < 1e-7);

    assert_true(std::abs(logistic_function(inverse_logistic_function(0.01)) -
                         0.01) < 1e-7);

    assert_true(std::abs(logistic_function(inverse_logistic_function(0.1)) -
                         0.1) < 1e-7);

    assert_true(std::abs(logistic_function(inverse_logistic_function(0.5)) -
                         0.5) < 1e-7);

    assert_true(std::abs(logistic_function(inverse_logistic_function(0.9)) -
                         0.9) < 1e-7);

    assert_true(std::abs(logistic_function(inverse_logistic_function(0.99)) -
                         0.99) < 1e-7);

    assert_true(std::abs(logistic_function(inverse_logistic_function(0.999)) -
                         0.999) < 1e-7);

    assert_true(std::abs(logistic_function(inverse_logistic_function(0.9999)) -
                         0.9999) < 1e-7);
  }
 ...
 public:
  ...
  void apply_transformation(std::vector<Float>* yhat_) const final {
    std::for_each(yhat_->begin(), yhat_->end(),
                  [this](Float& _val) { _val = logistic_function(_val); });
  }

  /// Calculates the sampling rate (the share of samples that will be
  /// drawn for each feature).
  void calc_sampling_rate(const bool _set_rate, const Float _sampling_factor,
    ...
    if (_set_rate) {
      ...
      sampler().calc_sampling_rate(targets().size(), _sampling_factor, _comm);
    }
  }

  /// Calculates sum_g_ and sum_h_.
  void calc_sums() {
    impl_.calc_sums(sample_index_, *sample_weights_, &sum_g_, &sum_h_,
                    &sum_sample_weights_, &comm());
  }
  ...
  void calc_etas(const enums::Aggregation _agg, const enums::Update _update,
    ...
    impl_.calc_sufficient_stats(_agg, _update, _old_weights, _indices_current,
                                _eta1, _eta1_old, _eta2, _eta2_old,
                                yhat_committed_, &sufficient_stats_);
  }
  ...
  std::pair<Float, containers::Weights> calc_pair(
      ...
      const std::vector<Float>& _eta2,
      const std::vector<Float>& _eta2_old) final {
    assert_true(_update != enums::Update::calc_all ||
                _indices_current.size() == _indices.size());

    return impl_.calc_pair(_agg, _revert, _update, _old_weights,
                           _indices_current, _eta1, _eta1_old, _eta2, _eta2_old,
                           yhat_committed_, &sufficient_stats_, &comm());
  }
  ...
  std::pair<Float, containers::Weights> calc_pair(
    ...
    return impl_.calc_pair(_revert, _update, _begin, _split_begin, _split_end,
                           _end, &loss_old_, &sufficient_stats_xgboost_,
                           &comm());
  }
  ...
  void calc_yhat(const enums::Aggregation _agg,
                 ...
                 const std::vector<Float>& _eta2,
                 const std::vector<Float>& _eta2_old) final {
    impl_.calc_yhat(_agg, _old_weights, _new_weights, _indices, _eta1, _eta2,
                    yhat_committed_, &yhat_);
  }
  ...
  Float evaluate_split(const Float _old_intercept,
                       const std::vector<Float>& _old_weights,
                       const containers::Weights& _weights) final {
  ...
  Float evaluate_split(
      const Float _old_intercept, const std::vector<Float>& _old_weight,
  ...
  Float evaluate_tree(const Float _update_rate,
  ...
  void resize(const size_t _nrows, const size_t _ncols) final {
    g_.resize(_nrows);
    h_.resize(_nrows);

    yhat_.resize(_nrows);
    yhat_committed_.resize(_nrows);
  ...
  /// columns.
  void revert(const std::vector<Float>& _old_weights) final {
  ...
  /// Trivial setter.
  void set_comm(multithreading::Communicator* _comm) final { comm_ = _comm; }
 ...
 private:
  ...
  Float calc_loss(const containers::Weights& _weights) {
    assert_true(sample_weights_);
    return impl_.calc_loss(1.0, std::get<0>(_weights), yhat_, &comm());
  }
 ...
 private:
  ...
  /// The sufficient statistics needed to apply the relmt formula
  /// (needed for calc_pair).
  std::vector<Float> sufficient_stats_;
...
#endif  // RELMT_LOSSFUNCTIONS_CROSSENTROPYLOSS_HPP_
