#ifndef RELMT_LOSSFUNCTIONS_LOSSFUNCTION_HPP_
#define RELMT_LOSSFUNCTIONS_LOSSFUNCTION_HPP_
...
class LossFunction {
 public:
  ...
  virtual void calc_etas(const enums::Aggregation _agg,
  ...
  virtual void calc_sampling_rate(const bool _set_rate,
  ...
  virtual Float calc_update_rate(const std::vector<Float>& _predictions) = 0;

  /// Calculates the weights and the associated partial loss given values from
  /// a parent aggregation.
  virtual std::pair<Float, containers::Weights> calc_pair(
      ...
      const std::vector<Float>& _eta2, const std::vector<Float>& _eta2_old) = 0;
  ...
  virtual void calc_yhat(const enums::Aggregation _agg,
                         const std::vector<Float>& _old_weights,
  ...
  virtual std::shared_ptr<const lossfunctions::LossFunction> child() const = 0;

  /// Commits the values for yhat_old_.
  virtual void commit() = 0;

  /// Commits the current values.
  virtual void commit(const Float _old_intercept,
                      const std::vector<Float>& _old_weights,
                      const containers::Weights& _weights) = 0;

  /// Commits the values described by the indices and yhat.
  virtual void commit(const std::vector<size_t>& _indices,
                      const containers::Weights& _weights) = 0;
  ...
  virtual Float evaluate_split(const Float _old_intercept,
                               const std::vector<Float>& _old_weights,
                               const containers::Weights& _weights) = 0;
  ...
  virtual Float evaluate_tree(const Float _update_rate,
  ...
  virtual void resize(const size_t _nrows, const size_t _ncols) = 0;
  ...
  /// columns.
  virtual void revert(const std::vector<Float>& _old_weights) = 0;
  ...
  virtual void update_yhat_old(const Float _update_rate,
...
#endif  // RELMT_LOSSFUNCTIONS_LOSSFUNCTION_HPP_
