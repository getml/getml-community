#ifndef MULTIREL_OPTIMIZATIONCRITERION_HPP_
#define MULTIREL_OPTIMIZATIONCRITERION_HPP_
...
class OptimizationCriterion {
 public:
  ...
  virtual std::vector<size_t> argsort(const size_t _begin,
                                      const size_t _end) const = 0;
  ...
  virtual void init_yhat(const std::vector<Float>& _yhat,
  ...
  virtual std::shared_ptr<std::vector<Float>> make_sample_weights() = 0;
  ...
  virtual void store_current_stage(const Float _num_samples_smaller,
                                   const Float _num_samples_greater) = 0;

  /// Updates all samples designated by _indices
  virtual void update_samples(const containers::IntSet& _indices,
                              const std::vector<Float>& _new_values,
                              const std::vector<Float>& _old_values) = 0;

  /// Updates yhat_old based on _yhat_new.
  virtual void update_yhat_old(const std::vector<Float>& _sample_weights,
...
#endif  // MULTIREL_OPTIMIZATIONCRITERION_HPP_
