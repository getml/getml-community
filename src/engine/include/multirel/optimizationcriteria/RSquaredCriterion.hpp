...
#include "multirel/optimizationcriteria/OptimizationCriterion.hpp"
...
class RSquaredCriterion : public OptimizationCriterion {
 public:
  RSquaredCriterion(const std::shared_ptr<const descriptors::Hyperparameters>&
                        _hyperparameters,
  ...
  void reset() final {
    impl().reset(&sufficient_statistics_current_,
  ...
  void revert_to_commit() final {
    assert_true(sufficient_statistics_current_.size() ==
                sufficient_statistics_committed_.size());
    std::copy(sufficient_statistics_committed_.begin(),
  ...
  void store_current_stage(const Float _num_samples_smaller,
    ...
    impl().store_current_stage(_num_samples_smaller, _num_samples_greater,
