...
class DecisionTree {
 public:
  DecisionTree(const std::shared_ptr<const descriptors::TreeHyperparameters>
                   &_tree_hyperparameters,
               const Poco::JSON::Object &_json_obj);

  DecisionTree(const std::string &_agg,
 ...
 public:
  ...
  inline std::shared_ptr<aggregations::AbstractFitAggregation> make_aggregation(
      const containers::DataFrameView &_population,
      const containers::DataFrame &_peripheral,
      const containers::Subfeatures &_subfeatures,
  ...
  /// a commit)
  inline void store_current_stage(const Float _num_samples_smaller,
 ...
 private:
  ...
  /// Subtrees
  inline const std::vector<DecisionTree> &subtrees() const { return subtrees_; }
