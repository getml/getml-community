#ifndef MULTIREL_DESCRIPTORS_SPLIT_HPP_
#define MULTIREL_DESCRIPTORS_SPLIT_HPP_
...
struct Split {
  ...
  Split(const bool _apply_from_above, const Float _critical_value,
  ...
  Split(const bool _apply_from_above,
  ...
  Split(const Poco::JSON::Object &_json_obj)
      : apply_from_above(JSON::get_value<bool>(_json_obj, "app_")),
        ...
        critical_value(JSON::get_value<Float>(_json_obj, "critical_value_")),
  ...
  Split deep_copy() const {
    auto sorted = std::make_shared<std::vector<Int>>(categories_used_begin,
                                                     categories_used_end);

    std::sort(sorted->begin(), sorted->end());

    return Split(apply_from_above, critical_value, sorted, column_used,
                 data_used);
  }
...
#endif  // MULTIREL_DESCRIPTORS_SPLIT_HPP_
