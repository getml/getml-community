...
class TransformAggregation : public AbstractTransformAggregation {
 ...
 public:
  ...
  Float aggregate(
      ...
      const std::optional<containers::Column<Float>>& _time_stamp) const final;
...
Float TransformAggregation<AggType, data_used_, is_population_>::aggregate(
  ...
  if constexpr (std::is_same<AggType, AggregationType::CountDistinct>()) {
    return helpers::Aggregations::count_distinct(range.begin(), range.end());
  }

  if constexpr (std::is_same<AggType,
                             AggregationType::CountMinusCountDistinct>()) {
    return helpers::Aggregations::count(range.begin(), range.end()) -
           helpers::Aggregations::count_distinct(range.begin(), range.end());
  }
...
auto TransformAggregation<AggType, data_used_, is_population_>::make_range(
  ...
  if constexpr (is_count) {
    const auto get_value = [](const containers::Match* _match) -> Float {
      return 0.0;
    };

    return _match_ptrs | VIEWS::drop(_skip) | VIEWS::transform(get_value);
  }
  ...
  if constexpr (is_first_or_last) {
    ...
    const auto get_pair =
        [this, _time_stamp](
            const containers::Match* _match) -> std::pair<Float, Float> {
      const auto first = _time_stamp.value()[_match->ix_x_perip];
      const auto second = value_container_.value_to_be_aggregated(_match);
      return std::pair(first, second);
    };

    return _match_ptrs | VIEWS::drop(_skip) | VIEWS::transform(get_pair);
  }

  if constexpr (!is_first_or_last && !is_count) {
    const auto get_value = [this](const containers::Match* _match) -> Float {
...
TransformAggregation<AggType, data_used_, is_population_>::separate_null_values(
  ...
  if constexpr (!is_count) {
    ...
    return std::partition(_match_ptrs->begin(), _match_ptrs->end(), is_null);
  }
