...
#include "multirel/aggregations/Revert.hpp"
...
class FitAggregation : public AbstractFitAggregation {
 public:
  FitAggregation(const FitAggregationParams &_params)
      : AbstractFitAggregation(),
        ...
        value_container_(
            ValueContainerCreator<data_used_, is_population_>::create(
                _params.same_units_discrete_, _params.same_units_numerical_,
                _params.column_to_be_aggregated_, _params.population_,
                _params.peripheral_, _params.subfeatures_)) {
    ...
    if constexpr (needs_sorting_) {
      sort_matches(_params.peripheral_, null_value_separator,
  ...
  void activate_all(const bool _init_opt,
  ...
  void activate_matches_containing_categories(
      ...
      const Revert _revert, const containers::CategoryIndex &_index) final;
  ...
  void activate_matches_containing_words(
      const std::vector<Int>::const_iterator _categories_begin,
      const std::vector<Int>::const_iterator _categories_end,
      const Revert _revert, const containers::WordIndex &_index) final;
  ...
  void activate_matches_not_containing_words(
      ...
      const std::vector<Int>::const_iterator _words_end, const Revert _revert,
  ...
  void update_optimization_criterion_and_clear_updates_current(
      const Float _num_matches_smaller, const Float _num_matches_greater) final;
  ...
  template <
      ...
      typename std::enable_if<std::is_same<Agg, AggregationType::Count>::value,
                              int>::type = 0>
  inline void activate_match(containers::Match *_match) {
  ...
  template <
      typename Agg = AggType,
      typename std::enable_if<std::is_same<Agg, AggregationType::Median>::value,
                              int>::type = 0>
  inline void activate_match(containers::Match *_match) {
    assert_true(_match->activated == false);
    ...
    const auto count = static_cast<Int>(this->count()[_match->ix_x_popul]);
    ...
    if (count % 2 == 0) {
      ...
      yhat_inline()[_match->ix_x_popul] = (value_to_be_aggregated(it_greater) +
      ...
      yhat_inline()[_match->ix_x_popul] = value_to_be_aggregated(it);
    }
  }
  ...
  inline void calculate_skewness(containers::Match *_match) {
    if (count()[_match->ix_x_popul] == 0.0) {
      ...
      const Float stddev = std::sqrt(sum_squared()[_match->ix_x_popul] /
                                         count()[_match->ix_x_popul] -
                                     mean * mean);

      const Float skewness =
          ((sum_cubed()[_match->ix_x_popul] / count()[_match->ix_x_popul]) -
  ...
  inline void activate_match(containers::Match *_match) {
    ...
    const Float mean = sum()[_match->ix_x_popul] / count()[_match->ix_x_popul];
    ...
    yhat_inline()[_match->ix_x_popul] =
        (yhat_inline()[_match->ix_x_popul] != yhat_inline()[_match->ix_x_popul])
            ? (0.0)
            : (yhat_inline()[_match->ix_x_popul]);
  }
 ...
 private:
  ...
  /// population table)
  void sort_matches(const containers::DataFrame &_peripheral,
  ...
  void sort_matches_by_ts(const containers::DataFrame &_peripheral,
                          containers::Matches::iterator _matches_begin,
                          containers::Matches::iterator _matches_end) const;
 ...
 private:
  ...
  inline Float value_to_be_aggregated(const containers::Match *_match) const {
    ...
    if constexpr (!std::is_same<ReturnType, Float>()) {
      static_assert(std::is_same<AggType, AggregationType::Count>(),
  ...
  /// Trivial accessor
  inline std::vector<Float> &yhat_inline() { return aggregation_impl_->yhat_; }
...
void FitAggregation<AggType, data_used_, is_population_>::
    activate_matches_containing_categories(
        const std::vector<Int>::const_iterator _categories_begin,
        const std::vector<Int>::const_iterator _categories_end,
        const Revert _revert, const containers::CategoryIndex &_index) {
  // ------------------------------------------------------------------
  ...
  for (auto cat = _categories_begin; cat < _categories_end; ++cat) {
    for (auto it = _index.begin(*cat); it < _index.end(*cat); ++it) {
      activate_match(*it);
...
void FitAggregation<AggType, data_used_, is_population_>::
  ...
  for (size_t i = _indptr.size() - 1; i > 0; --i) {
    ...
    const auto num_samples_greater = static_cast<Float>(_indptr[i]) + num_nans;
...
template <typename AggType, enums::DataUsed data_used_, bool is_population_>
void FitAggregation<AggType, data_used_, is_population_>::
    activate_matches_in_window(const std::vector<size_t> &_indptr,
  ...
  for (size_t i = 1; i < _indptr.size(); ++i) {
    assert_true(_indptr[i - 1] <= _indptr[i]);
    assert_true(_matches_begin <= _matches_end);
    assert_true(_indptr[i] <= static_cast<size_t>(
                                  std::distance(_matches_begin, _matches_end)));
    assert_true(_indptr[i] <= _indptr.back());
...
void FitAggregation<AggType, data_used_, is_population_>::
  ...
  const auto num_samples_smaller =
      static_cast<Float>(std::distance(_match_container_begin, _separator));

  const auto num_samples_greater =
      static_cast<Float>(std::distance(_separator, _match_container_end));

  update_optimization_criterion_and_clear_updates_current(num_samples_smaller,
                                                          num_samples_greater);
}
...
template <typename AggType, enums::DataUsed data_used_, bool is_population_>
void FitAggregation<AggType, data_used_, is_population_>::
    deactivate_matches_in_window(const std::vector<size_t> &_indptr,
                                 containers::MatchPtrs::iterator _matches_begin,
                                 containers::MatchPtrs::iterator _matches_end) {
  // ------------------------------------------------------------------
...
FitAggregation<AggType, data_used_, is_population_>::separate_null_values(
  ...
  if (std::is_partitioned(_match_ptrs->begin(), _match_ptrs->end(), is_null)) {
    return std::partition_point(_match_ptrs->begin(), _match_ptrs->end(),
                                is_null);
  } else {
    return std::stable_partition(_match_ptrs->begin(), _match_ptrs->end(),
                                 is_null);
  }
}
...
void FitAggregation<AggType, data_used_, is_population_>::sort_matches(
  ...
  const auto compare_op = [this](const containers::Match &match1,
    ...
    return value_to_be_aggregated(&match1) < value_to_be_aggregated(&match2);
  };
...
void FitAggregation<AggType, data_used_, is_population_>::sort_matches_by_ts(
  ...
  const auto retrieve_ts = [&ts_col](const containers::Match &match) -> Float {
...
void FitAggregation<AggType, data_used_, is_population_>::reset() {
  ...
  if constexpr (needs_count_) {
    ...
    std::fill(count_committed().begin(), count_committed().end(), 0.0);
  }
...
void FitAggregation<AggType, data_used_, is_population_>::
  ...
  optimization_criterion()->update_samples(updates_current(),  // _indices
  ...
  optimization_criterion()->store_current_stage(_num_samples_smaller,
