...
class SubtreeHelper {
 ...
 private:
  ...
  static void fit_subensemble(
      ...
      optimizationcriteria::OptimizationCriterion* _opt,
      multithreading::Communicator* _comm, DecisionTreeEnsemble* _subfeature);
};
...
void SubtreeHelper::fit_subensemble(
    ...
    const size_t _ix_perip_used,
    optimizationcriteria::OptimizationCriterion* _opt,
    multithreading::Communicator* _comm, DecisionTreeEnsemble* _subensemble) {
  ...
  const auto aggregation_index = aggregations::AggregationIndex(
      input_table, _table_holder->main_tables().at(_ix_perip_used), input_map,
      _output_map);

  const auto opt_impl =
      std::make_shared<aggregations::IntermediateAggregationImpl>(
          _table_holder->main_tables().at(0).nrows(), aggregation_index, _opt);

  const auto intermediate_agg =
      std::make_shared<aggregations::IntermediateAggregation<AggType>>(
          opt_impl);

  _subensemble->fit(subtable_holder, _word_indices, _logger,
                    _hyperparameters.num_subfeatures_, intermediate_agg, _comm);
