...
class FastProp {
 ...
 private:
  ...
  void build_rows(const TransformParams& _params,
                  ...
                  const size_t _thread_num, std::atomic<size_t>* _num_completed,
  ...
  containers::Features init_features(const size_t _nrows,
                                     const size_t _ncols) const;
  ...
  select_features(const FitParams& _params,
                  const std::shared_ptr<std::vector<size_t>>& _rownums) const;
  ...
  void spawn_threads(const TransformParams& _params,
                     const std::vector<containers::Features>& _subfeatures,
                     const std::shared_ptr<std::vector<size_t>>& _rownums,
                     containers::Features* _features) const;
 ...
 public:
  ...
  const std::vector<helpers::Schema>& peripheral_schema() const {
    throw_unless(peripheral_schema_,
 ...
 private:
  ...
  /// to generate the SQL code. In the other algorithms, this information is
  /// held by the individual trees.
  std::shared_ptr<const std::vector<helpers::Schema>> peripheral_table_schemas_;
