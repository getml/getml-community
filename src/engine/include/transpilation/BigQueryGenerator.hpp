#ifndef TRANSPILATION_BIGQUERYGENERATOR_HPP_
#define TRANSPILATION_BIGQUERYGENERATOR_HPP_
...
class BigQueryGenerator : public SQLDialectGenerator {
 public:
  explicit BigQueryGenerator(const TranspilationParams& _params)
      : params_(_params) {}

  ~BigQueryGenerator() = default;
 ...
 public:
  ...
  /// Removes the Macros from the colname and replaces it with proper PostgreSQL
  /// code.
  ...
  /// Transpiles the features in PostgreSQL code. This
 ...
 private:
  ...
  /// Edits the pre- and postfix to account for the way PostgreSQL deals with
  /// time intervals.
  ...
  /// Generates the CLUSTER BY statement, which serves to speed up BigQuery
  /// code.
  std::string make_cluster(const helpers::Schema& _schema) const;
...
#endif  // TRANSPILATION_BIGQUERYGENERATOR_HPP_
