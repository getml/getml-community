...
class Mapping : public Preprocessor {
 ...
 public:
  ...
  typedef std::vector<std::shared_ptr<const std::map<Int, std::vector<Float>>>>
      MappingForDf;
 ...
 public:
  ...
  Mapping(const Poco::JSON::Object& _obj,
 ...
 private:
  ...
  Float aggregate(const IteratorType _begin, const IteratorType _end,
  ...
  build_prerequisites(const containers::DataFrame& _population_df,
  ...
  MappingForDf extract_mapping(const Poco::JSON::Object& _obj,
  ...
  Mapping fit_on_table_holder(
      ...
      const std::vector<helpers::DataFrame>& _main_tables,
      const std::vector<helpers::DataFrame>& _peripheral_tables,
      const size_t _ix, logging::ProgressLogger* _logger) const;
  ...
  handle_text_fields(const helpers::DataFrame& _population,
  ...
  std::vector<std::pair<Int, Float>> make_pairs(const Map& _m,
                                                const size_t _weight_num) const;
  ...
  void transform_data_frame(const helpers::DataFrame& _immutable,
  ...
  Poco::JSON::Array::Ptr transform_mapping(const MappingForDf& _mapping) const;
 ...
 private:
  ...
  std::vector<std::string> columns_to_sql(
      const MappingToSqlType& _mapping_to_sql, const MappingType& _mappings,
  ...
  bool parse_subroles(const helpers::Column<T>& _col) const {
    const auto blacklist = std::vector<helpers::Subrole>(
        {helpers::Subrole::exclude_preprocessors, helpers::Subrole::email_only,
         helpers::Subrole::substring_only, helpers::Subrole::exclude_mapping});

    return !helpers::SubroleParser::contains_any(_col.subroles_, blacklist);
  }
