...
class DataFrame {
 ...
 public:
  ...
  void from_db(const std::shared_ptr<database::Connector> _connector,
  ...
  void from_query(const std::shared_ptr<database::Connector> _connector,
...
DataFrameType DataFrame::to_immutable(const std::optional<Schema> &_schema,
  ...
  const auto targets = _targets
                           ? stl::collect::vector<FloatColumnType>(
