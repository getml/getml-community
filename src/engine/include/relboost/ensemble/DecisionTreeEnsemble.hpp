...
class DecisionTreeEnsemble {
 ...
 public:
  ...
  const Hyperparameters& hyperparameters() const {
    throw_unless(impl().hyperparameters_, "Model has no hyperparameters.");
  ...
  const std::vector<helpers::Schema>& peripheral_schema() const {
    throw_unless(impl().peripheral_schema_,
 ...
 private:
  ...
  void extract_schemas(const containers::DataFrame& _population,
                       const std::vector<containers::DataFrame>& _peripheral);
 ...
 private:
  ...
  /// Trivial (private) accessor
  const Float initial_prediction() const { return impl().initial_prediction_; }
  ...
  void update_predictions(const Float _update_rate,
