...
class Avg : public lossfunctions::LossFunction {
 ...
 public:
  ...
  Avg(const std::shared_ptr<lossfunctions::LossFunction>& _child,
      ...
      const containers::DataFrameView& _output, const bool _allow_null_weights,
  ...
  Avg(const std::shared_ptr<AggregationIndex>& _agg_index,
      ...
      multithreading::Communicator* _comm)
      : Avg(_child, std::vector<containers::Match>(0), _input, _output, false,
            _comm) {
 ...
 private:
  ...
  /// the last split.
  void calc_all(const enums::Revert _revert, const Float _old_weight,
                const std::vector<containers::Match>::iterator _begin,
                const std::vector<containers::Match>::iterator _split_begin,
                const std::vector<containers::Match>::iterator _split_end,
  ...
  void calc_diff(const Float _old_weight,
                 const std::vector<containers::Match>::iterator _split_begin,
                 const std::vector<containers::Match>::iterator _split_end);

  /// Calculates the new yhat assuming that this is the
  /// lowest aggregation.
  void calc_yhat(const Float _old_weight,
                 const std::array<Float, 3>& _new_weights);

  /// Dectivates a set of indices by decreasing the number of counts.
  void deactivate(const Float _old_weight,
