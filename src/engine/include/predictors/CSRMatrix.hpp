...
template <typename DataType = Float, typename IndicesType = size_t,
...
class CSRMatrix {
 ...
 public:
  ...
  CSRMatrix(const FloatFeature& _col);

  /// Constructs a CSRMatrix from a categorical column.
  CSRMatrix(const IntFeature& _col, const size_t _n_unique);

  // -----------------------------------------------------------

 public:
  /// Adds a discrete or numerical column.
  void add(const FloatFeature& _col);

  /// Adds a categorical column.
  void add(const IntFeature& _col, const size_t _n_unique);
...
template <typename DataType, typename IndicesType, typename IndptrType>
CSRMatrix<DataType, IndicesType, IndptrType>::CSRMatrix(
    const IntFeature& _col, const size_t _n_unique) {
  indptr_ = std::vector<IndptrType>(_col.size() + 1);
...
template <typename DataType, typename IndicesType, typename IndptrType>
void CSRMatrix<DataType, IndicesType, IndptrType>::add(const IntFeature& _col,
                                                       const size_t _n_unique) {
  // -------------------------------------------------------------------------
