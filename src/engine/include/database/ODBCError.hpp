...
class ODBCError {
 public:
  ...
  static void check(const SQLRETURN _ret, const std::string& _activity,
                    const SQLHANDLE _handle, const SQLSMALLINT _type) {
 ...
 private:
  ...
  /// Throws an error
  static void throw_error(const SQLRETURN _ret, const std::string& _activity,
                          const SQLHANDLE _handle, const SQLSMALLINT _type) {
    std::string err_msg =
    ...
    for (SQLINTEGER i = 1; true; ++i) {
      ...
      const auto ret =
          SQLGetDiagRec(_type, _handle, i, state, &native_error, diag_rec,
                        sizeof(diag_rec), &diag_rec_length);

      if (SQL_SUCCEEDED(ret)) {
        err_msg += std::to_string(i) + ":" + std::to_string(native_error) +
