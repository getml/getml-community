...
struct ODBCEnv {
  ODBCEnv() : handle_(SQL_NULL_HENV) {
    ...
    ODBCError::check(ret, "SQLAllocHandle(SQL_HANDLE_ENV)", handle_,
                     SQL_HANDLE_ENV);

    ret = SQLSetEnvAttr(handle_, SQL_ATTR_ODBC_VERSION,
                        (SQLPOINTER*)SQL_OV_ODBC3, 0);

    ODBCError::check(ret, "SQLSetEnvAttr(SQL_HANDLE_ENV)", handle_,
                     SQL_HANDLE_ENV);
  }
  ...
  ODBCEnv(ODBCEnv&& _other) noexcept { handle_ = std::move(_other.handle_); }
