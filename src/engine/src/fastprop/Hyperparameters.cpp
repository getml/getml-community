...
Hyperparameters::Hyperparameters(const Poco::JSON::Object& _json_obj)
    : aggregations_(jsonutils::JSON::array_to_vector<std::string>(
      ...
      delta_t_(_json_obj.has("delta_t_")
                   ? jsonutils::JSON::get_value<Float>(_json_obj, "delta_t_")
                   : 0.0),
      loss_function_(
          jsonutils::JSON::get_value<std::string>(_json_obj, "loss_function_")),
      max_lag_(_json_obj.has("max_lag_")
      ...
      num_threads_(jsonutils::JSON::get_value<Int>(_json_obj, "num_threads_")),
      sampling_factor_(
          _json_obj.has("sampling_factor_")
              ? jsonutils::JSON::get_value<Float>(_json_obj, "sampling_factor_")
              : 1.0),
      silent_(jsonutils::JSON::get_value<bool>(_json_obj, "silent_")),
      vocab_size_(
          jsonutils::JSON::get_value<size_t>(_json_obj, "vocab_size_")) {
  assert_true(loss_function_ == CROSS_ENTROPY_LOSS ||
              loss_function_ == SQUARE_LOSS);
}
...
Poco::JSON::Object::Ptr Hyperparameters::to_json_obj() const {
  ...
  obj->set("aggregation_", jsonutils::JSON::vector_to_array_ptr(aggregations_));
