...
std::string SQLite3Generator::create_table(
...
SQLite3Generator::demangle_colname(const std::string& _raw_name) const {
  ...
  new_name = helpers::StringReplacer::replace_all(
      ...
      "strftime('%H', " + helpers::Macros::prefix());
  ...
  new_name = helpers::StringReplacer::replace_all(
      ...
      "strftime('%M', " + helpers::Macros::prefix());
  ...
  new_name = helpers::StringReplacer::replace_all(
      ...
      "strftime('%w', " + helpers::Macros::prefix());
  ...
  new_name = helpers::StringReplacer::replace_all(
      ...
      "strftime('%Y', " + helpers::Macros::prefix());
...
std::string SQLite3Generator::make_staging_table_colname(
...
std::string SQLite3Generator::join_mapping(const std::string& _name,
  ...
  const auto to_epoch_time_or_rowid =
    ...
    const auto epoch_time =
        is_rowid(_colname)
            ...
            : "( julianday( " + make_staging_table_column(_colname, "t1") +
...
std::string SQLite3Generator::make_mapping_table_insert_into(
...
std::string SQLite3Generator::string_contains(const std::string& _colname,
