#include "relmt/utils/CriticalValueSorter.hpp"
...
std::pair<Float, Float> CriticalValueSorter::calc_average(
  ...
  const auto [sums, count] =
      calc_sums(_output_rescaled, _input_rescaled, _split_begin, _split_end);
...
std::vector<Float> CriticalValueSorter::make_averages(
  ...
  auto averages =
      std::vector<Float>(std::distance(_candidates_begin, _candidates_end));
  ...
  if (_row_index) {
    make_averages_words(averages.size(), total_sums, total_count, *_row_index,
                        _indptr, _output_rescaled, _input_rescaled,
                        _candidates_begin, _bins, sums, counts);
  } else {
    make_averages_category(averages.size(), total_sums, total_count, _min,
                           _indptr, _output_rescaled, _input_rescaled,
                           _candidates_begin, _bins, sums, counts);
  }
...
void CriticalValueSorter::make_averages_category(
  ...
  for (size_t i = 0; i < _size; ++i) {
    ...
    std::tie(_sums[i], _counts[i]) =
        calc_average(*(_candidates_begin + i), _total_sums, _total_count,
                     _output_rescaled, _input_rescaled, split_begin, split_end);
  }
}
...
void CriticalValueSorter::make_averages_words(
  ...
  for (size_t i = 0; i < _size; ++i) {
    const auto word = *_candidates_begin[i].split_.categories_used_begin_;

    textmining::Matches::extract(word, _row_index, _indptr, _bins->begin(),
                                 _bins->end(), &extracted);

    std::tie(_sums[i], _counts[i]) = calc_average(
        *(_candidates_begin + i), _total_sums, _total_count, _output_rescaled,
        _input_rescaled, extracted.begin(), extracted.end());
  }
}
...
std::vector<std::tuple<Float, Int>> CriticalValueSorter::make_tuples(
  ...
  assert_true(static_cast<size_t>(std::distance(_begin, _end)) ==
              _averages.size());
...
std::shared_ptr<const std::vector<Int>> CriticalValueSorter::sort(
  ...
  const auto averages = make_averages(
      _min, _row_index, _indptr, _output_rescaled, _input_rescaled,
      _candidates_begin, _candidates_end, _bins, _comm);
