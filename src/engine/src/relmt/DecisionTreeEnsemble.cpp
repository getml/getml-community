...
DecisionTreeEnsemble::DecisionTreeEnsemble(
    ...
    : impl_(DecisionTreeEnsembleImpl(_hyperparameters, _peripheral,
                                     _placeholder)),
...
DecisionTreeEnsemble::DecisionTreeEnsemble(const Poco::JSON::Object &_obj)
  ...
  initial_prediction() = JSON::get_value<Float>(_obj, "initial_prediction_");
  ...
  if (_obj.has("peripheral_schema_")) {
    impl().peripheral_schema_ = helpers::Schema::from_json(
  ...
  for (size_t i = 0; i < trees_objects.size(); ++i) {
    const auto ptr = trees_objects.getObject(static_cast<unsigned int>(i));

    if (!ptr) {
      throw std::runtime_error("Tree " + std::to_string(i) +
                               " is not a valid JSON object!");
    }

    trees().push_back(decisiontrees::DecisionTree(impl().hyperparameters_,
                                                  loss_function_, *ptr));
  }
  ...
  subensembles_avg_ =
      std::vector<std::optional<DecisionTreeEnsemble>>(features_avg->size());

  for (size_t i = 0; i < features_avg->size(); ++i) {
    auto obj = features_avg->getObject(static_cast<unsigned int>(i));

    if (obj) {
      subensembles_avg_[i] = std::make_optional<DecisionTreeEnsemble>(*obj);
    }
  }
  ...
  subensembles_sum_ =
      std::vector<std::optional<DecisionTreeEnsemble>>(features_sum->size());

  for (size_t i = 0; i < features_sum->size(); ++i) {
    auto obj = features_sum->getObject(static_cast<unsigned int>(i));

    if (obj) {
      subensembles_sum_[i] = std::make_optional<DecisionTreeEnsemble>(*obj);
    }
  }
...
void DecisionTreeEnsemble::calc_initial_prediction() {
  auto sum = std::accumulate(targets().begin(), targets().end(), 0.0,
                             std::plus<Float>());
...
void DecisionTreeEnsemble::fit_candidate(
    ...
    std::vector<std::tuple<decisiontrees::DecisionTree, Float,
                           std::vector<Float>>> *_candidates) const {
  // Reset loss_function_committed_.
  _loss_function->commit();

  auto new_candidate =
      decisiontrees::DecisionTree(impl().hyperparameters_, _agg, _ix_table_used,
                                  _output_scaler, _input_scaler, &comm());

  new_candidate.fit(_output_table, _input_table, _subfeatures, _output_rescaled,
                    _input_rescaled, _matches->begin(), _matches->end());
  ...
  _loss_function->reduce_predictions(new_candidate.intercept(),
                                     &new_predictions);
  ...
  _candidates->emplace_back(std::make_tuple(std::move(new_candidate), new_loss,
...
DecisionTreeEnsemble::fit_candidate_features(
  ...
  for (size_t ix_table_used = 0;
       ix_table_used < _table_holder->main_tables().size(); ++ix_table_used) {
    // ------------------------------------------------------------------------
    ...
    const auto &output_table = _table_holder->main_tables().at(ix_table_used);
    ...
    const auto [output_rescaled, input_rescaled, output_scaler, input_scaler] =
        fit_transform_scalers(output_table, *input_table, subfeatures,
                              matches.begin(), matches.end());

    // ------------------------------------------------------------------------

    const auto aggregations =
        make_aggregations(_loss_function, matches, *input_table, output_table,
                          input_rescaled, output_rescaled);

    // ------------------------------------------------------------------------

    for (auto &agg : aggregations) {
      fit_candidate(ix_table_used, agg, _loss_function, input_scaler,
                    output_scaler, output_table, *input_table, subfeatures,
                    *output_rescaled, *input_rescaled, &matches, &candidates);
    }
...
void DecisionTreeEnsemble::fit_new_features(
  ...
  auto candidates = fit_candidate_features(_loss_function, _table_holder,
                                           _subfeatures, sample_weights);
...
void DecisionTreeEnsemble::fit_subensembles(
  ...
  SubtreeHelper::fit_subensembles(_table_holder, _logger, *this, _loss_function,
                                  &comm(), &subensembles_avg_,
                                  &subensembles_sum_);
}

// ----------------------------------------------------------------------------

std::tuple<std::optional<containers::Rescaled>,
           std::optional<containers::Rescaled>,
           std::shared_ptr<const utils::StandardScaler>,
           std::shared_ptr<const utils::StandardScaler>>
DecisionTreeEnsemble::fit_transform_scalers(
    const containers::DataFrameView &_output,
    const containers::DataFrame &_input,
    const containers::Subfeatures &_subfeatures,
    const std::vector<containers::Match>::iterator _begin,
    const std::vector<containers::Match>::iterator _end) const {
  const auto output_scaler = std::make_shared<utils::StandardScaler>();

  const auto output_rescaled = std::make_optional<containers::Rescaled>(
      output_scaler->fit_transform(_output, std::nullopt, _begin, _end));

  const auto input_scaler = std::make_shared<utils::StandardScaler>();

  const auto input_rescaled = std::make_optional<containers::Rescaled>(
      input_scaler->fit_transform(_input, _subfeatures, _begin, _end));

  return std::make_tuple(output_rescaled, input_rescaled, output_scaler,
                         input_scaler);
}

// ----------------------------------------------------------------------------

std::pair<std::shared_ptr<lossfunctions::LossFunction>,
...
DecisionTreeEnsemble::make_aggregations(
  ...
  if (hyperparameters().allow_avg_) {
    aggregations.push_back(std::make_shared<aggregations::Avg>(
        _child, _matches, _input_table, _output_table, &comm(), _input_rescaled,
        _output_rescaled));
  }

  aggregations.push_back(std::make_shared<aggregations::Sum>(
      _child, _input_table, _output_table, &comm(), _input_rescaled,
      _output_rescaled));
...
std::vector<containers::Predictions> DecisionTreeEnsemble::make_subpredictions(
  ...
  return SubtreeHelper::make_predictions(_table_holder, subensembles_avg_,
                                         subensembles_sum_, _logger, _comm);
}
...
void DecisionTreeEnsemble::subfeatures_to_sql(
  ...
  for (size_t i = 0; i < subensembles_avg_.size(); ++i) {
    if (subensembles_avg_.at(i)) {
      ...
      const auto it =
          _peripheral_map->find(subensembles_avg_.at(i)->placeholder().name());
...
Poco::JSON::Object DecisionTreeEnsemble::to_json_obj(
  ...
  // ------------------------------------------------------------------------
  // When relmt is used as a predictor, it has no peripheral_ or
  // placeholder_.
  ...
  if (impl().population_schema_) {
    Poco::JSON::Array::Ptr peripheral_schema_arr(new Poco::JSON::Array());
...
std::vector<std::string> DecisionTreeEnsemble::to_sql(
  ...
  for (size_t i = 0; i < trees().size(); ++i) {
    ...
    sql.push_back(tree.to_sql(_categories, _vocabulary, _sql_dialect_generator,
