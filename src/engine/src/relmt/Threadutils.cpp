...
void Threadutils::fit_as_feature_learner(const ThreadutilsFitParams& _params) {
  ...
  const auto population_subview = utils::DataFrameScatterer::scatter_data_frame(
  ...
  const auto [loss_function, table_holder] = _params.ensemble_->init(
      population_subview, _params.peripheral_, _params.row_indices_,
      _params.word_indices_, _params.feature_container_);

  _params.ensemble_->fit_subensembles(table_holder, _params.logger_,
                                      loss_function);
  ...
  utils::Logger::log("RelMT: Training features...", _params.logger_,
                     _params.comm_);

  while (_params.ensemble_->num_features() < num_features) {
    _params.ensemble_->fit_new_features(loss_function, table_holder,
                                        subfeatures, num_features);
...
void Threadutils::transform_as_feature_learner(
  ...
  auto subfeatures = SubtreeHelper::make_subfeatures(table_holder, predictions);

  assert_true(_params.features_->size() == _params.index_.size());

  utils::Logger::log("RelMT: Building features...", _params.logger_,
                     _params.comm_);
  ...
  for (size_t i = 0; i < _params.index_.size(); ++i) {
    ...
    copy(population_subview.rows(), *new_feature, &_params.features_->at(i));

    const auto progress = ((i + 1) * 100) / _params.index_.size();

    utils::Logger::log("Built FEATURE_" + std::to_string(ix + 1) +
