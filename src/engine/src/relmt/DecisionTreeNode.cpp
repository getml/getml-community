...
DecisionTreeNode::DecisionTreeNode(
    const utils::ConditionMaker& _condition_maker, const Int _depth,
    const std::shared_ptr<const Hyperparameters>& _hyperparameters,
    const std::shared_ptr<lossfunctions::LossFunction>& _loss_function,
    const std::shared_ptr<const helpers::Schema>& _input,
    const std::shared_ptr<const helpers::Schema>& _output,
    const std::vector<Float> _weights, multithreading::Communicator* _comm)
    : comm_(_comm),
...
DecisionTreeNode::DecisionTreeNode(
  ...
  if (_obj.has("child_greater_")) {
    const auto categories_used = std::make_shared<const std::vector<Int>>(
        JSON::array_to_vector<Int>(JSON::get_array(_obj, "categories_used_")));

    const auto column = JSON::get_value<size_t>(_obj, "column_");

    const auto column_input = JSON::get_value<size_t>(_obj, "column_input_");

    const auto critical_value = JSON::get_value<Float>(_obj, "critical_value_");

    const auto data_used =
        JSON::destringify(JSON::get_value<std::string>(_obj, "data_used_"));

    // For backwards compatatability.
    if (_obj.has("loss_reduction_")) {
      loss_reduction_ = JSON::get_value<Float>(_obj, "loss_reduction_");
    }

    split_ = containers::Split(categories_used, column, column_input,
                               critical_value, data_used);

    child_greater_.reset(new DecisionTreeNode(
        condition_maker_, depth_ + 1, hyperparameters_, loss_function_, input_,
        output_, *JSON::get_object(_obj, "child_greater_")));

    child_smaller_.reset(new DecisionTreeNode(
        condition_maker_, depth_ + 1, hyperparameters_, loss_function_, input_,
        output_, *JSON::get_object(_obj, "child_smaller_")));
  }
}
...
void DecisionTreeNode::add_candidates(
    ...
    const Float _old_intercept, const containers::Split& _split,
  ...
  auto [loss_reduction, new_weights] = loss_function().calc_pair(
      _revert, _update, hyperparameters().min_num_samples_, _old_intercept,
      weights_, _begin, _last_it, _it, _end);

  _candidates->push_back(containers::CandidateSplit(loss_reduction, _split,
...
void DecisionTreeNode::assert_aligned(
  ...
  utils::Reducer::reduce(
      ...
      &global_num_candidates, &comm());

  utils::Reducer::reduce(multithreading::maximum<decltype(global_ix_best)>(),
                         &global_ix_best, &comm());

  utils::Reducer::reduce(
      multithreading::maximum<decltype(global_partial_loss)>(),
      &global_partial_loss, &comm());
...
void DecisionTreeNode::column_importances(
  ...
  if (!std::isnan(loss_reduction_)) {
    ...
    _importance_maker->add(*input_, *output_, split_.data_used_, split_.column_,
                           split_.column_input_, loss_reduction_);
...
void DecisionTreeNode::fit(
    ...
    const containers::Rescaled& _input_rescaled,
    const std::vector<containers::Match>::iterator _begin,
    const std::vector<containers::Match>::iterator _end, Float* _intercept) {
  // ------------------------------------------------------------------------
  ...
  auto candidates = try_all(*_intercept, _output, *_input, _subfeatures,
                            _output_rescaled, _input_rescaled, _begin, _end);
  ...
  const auto it = std::min_element(candidates.begin(), candidates.end(), cmp);
  ...
  const auto it_split =
      partition(_output, _input, _subfeatures, best_split.split_, _begin, _end);
  ...
  *_intercept = std::get<0>(best_split.weights_);
  ...
  child_greater_.reset(new DecisionTreeNode(
      condition_maker_, depth_ + 1, hyperparameters_, loss_function_, input_,
      output_, std::get<1>(best_split.weights_), &comm()));

  child_smaller_.reset(new DecisionTreeNode(
      condition_maker_, depth_ + 1, hyperparameters_, loss_function_, input_,
      output_, std::get<2>(best_split.weights_), &comm()));

  child_greater_->fit(_output, _input, _subfeatures, _output_rescaled,
                      _input_rescaled, _begin, it_split, _intercept);

  child_smaller_->fit(_output, _input, _subfeatures, _output_rescaled,
                      _input_rescaled, it_split, _end, _intercept);
...
bool DecisionTreeNode::match_is_greater(
  ...
  switch (split_.data_used_) {
    case enums::DataUsed::categorical_input:
      assert_true(_input);
      return utils::Partitioner<enums::DataUsed::categorical_input>::is_greater(
          split_, *_input, _match);

    case enums::DataUsed::categorical_output:
      return utils::Partitioner<
          enums::DataUsed::categorical_output>::is_greater(split_, _output,
                                                           _match);

    case enums::DataUsed::discrete_input:
      assert_true(_input);
      return utils::Partitioner<enums::DataUsed::discrete_input>::is_greater(
          split_, *_input, _match);

    case enums::DataUsed::discrete_input_is_nan:
      assert_true(_input);
      return utils::Partitioner<
          enums::DataUsed::discrete_input_is_nan>::is_greater(split_.column_,
                                                              *_input, _match);

    case enums::DataUsed::discrete_output:
      return utils::Partitioner<enums::DataUsed::discrete_output>::is_greater(
          split_, _output, _match);

    case enums::DataUsed::discrete_output_is_nan:
      return utils::Partitioner<
          enums::DataUsed::discrete_output_is_nan>::is_greater(split_.column_,
                                                               _output, _match);

    case enums::DataUsed::numerical_input:
      assert_true(_input);
      return utils::Partitioner<enums::DataUsed::numerical_input>::is_greater(
          split_, *_input, _match);

    case enums::DataUsed::numerical_input_is_nan:
      assert_true(_input);
      return utils::Partitioner<
          enums::DataUsed::numerical_input_is_nan>::is_greater(split_.column_,
                                                               *_input, _match);

    case enums::DataUsed::numerical_output:
      return utils::Partitioner<enums::DataUsed::numerical_output>::is_greater(
          split_, _output, _match);

    case enums::DataUsed::numerical_output_is_nan:
      return utils::Partitioner<
          enums::DataUsed::numerical_output_is_nan>::is_greater(split_.column_,
                                                                _output,
                                                                _match);
    ...
    case enums::DataUsed::same_units_categorical:
      assert_true(_input);
      return utils::Partitioner<
          enums::DataUsed::same_units_categorical>::is_greater(split_, *_input,
                                                               _output, _match);
    ...
    case enums::DataUsed::same_units_discrete:
      assert_true(_input);
      return utils::Partitioner<
          enums::DataUsed::same_units_discrete>::is_greater(split_, *_input,
                                                            _output, _match);

    case enums::DataUsed::same_units_discrete_is_nan:
      assert_true(_input);
      return utils::Partitioner<enums::DataUsed::same_units_discrete_is_nan>::
          is_greater(split_.column_input_, split_.column_, *_input, _output,
                     _match);
    ...
    case enums::DataUsed::same_units_numerical:
      assert_true(_input);
      return utils::Partitioner<
          enums::DataUsed::same_units_numerical>::is_greater(split_, *_input,
                                                             _output, _match);

    case enums::DataUsed::same_units_numerical_is_nan:
      assert_true(_input);
      return utils::Partitioner<enums::DataUsed::same_units_numerical_is_nan>::
          is_greater(split_.column_input_, split_.column_, *_input, _output,
                     _match);

    case enums::DataUsed::subfeatures:
      assert_true(_input);
      return utils::Partitioner<enums::DataUsed::subfeatures>::is_greater(
          split_, _subfeatures, _match);

    case enums::DataUsed::text_input:
      assert_true(_input);
      return utils::Partitioner<enums::DataUsed::text_input>::is_greater(
          split_, *_input, _match);

    case enums::DataUsed::text_output:
      assert_true(_input);
      return utils::Partitioner<enums::DataUsed::text_output>::is_greater(
          split_, _output, _match);

    case enums::DataUsed::time_stamps_window:
      assert_true(_input);
      return utils::Partitioner<enums::DataUsed::time_stamps_window>::
          is_greater(split_, hyperparameters().delta_t_, *_input, _output,
                     _match);
...
std::vector<containers::Match>::iterator DecisionTreeNode::partition(
  ...
  switch (_split.data_used_) {
    case enums::DataUsed::categorical_input:
      assert_true(_input);
      return utils::Partitioner<enums::DataUsed::categorical_input>::partition(
          _split, *_input, _begin, _end);

    case enums::DataUsed::categorical_output:
      return utils::Partitioner<enums::DataUsed::categorical_output>::partition(
          _split, _output, _begin, _end);

    case enums::DataUsed::discrete_input:
      assert_true(_input);
      return utils::Partitioner<enums::DataUsed::discrete_input>::partition(
          _split, *_input, _begin, _end);

    case enums::DataUsed::discrete_input_is_nan:
      assert_true(_input);
      return utils::Partitioner<
          enums::DataUsed::discrete_input_is_nan>::partition(_split.column_,
                                                             *_input, _begin,
                                                             _end);

    case enums::DataUsed::discrete_output:
      return utils::Partitioner<enums::DataUsed::discrete_output>::partition(
          _split, _output, _begin, _end);

    case enums::DataUsed::discrete_output_is_nan:
      return utils::Partitioner<
          enums::DataUsed::discrete_output_is_nan>::partition(_split.column_,
                                                              _output, _begin,
                                                              _end);

    case enums::DataUsed::numerical_input:
      assert_true(_input);
      return utils::Partitioner<enums::DataUsed::numerical_input>::partition(
          _split, *_input, _begin, _end);

    case enums::DataUsed::numerical_input_is_nan:
      assert_true(_input);
      return utils::Partitioner<
          enums::DataUsed::numerical_input_is_nan>::partition(_split.column_,
                                                              *_input, _begin,
                                                              _end);

    case enums::DataUsed::numerical_output:
      return utils::Partitioner<enums::DataUsed::numerical_output>::partition(
          _split, _output, _begin, _end);

    case enums::DataUsed::numerical_output_is_nan:
      return utils::Partitioner<
          enums::DataUsed::numerical_output_is_nan>::partition(_split.column_,
                                                               _output, _begin,
                                                               _end);
    ...
    case enums::DataUsed::same_units_categorical:
      assert_true(_input);
      return utils::Partitioner<
          enums::DataUsed::same_units_categorical>::partition(_split, *_input,
                                                              _output, _begin,
                                                              _end);
    ...
    case enums::DataUsed::same_units_discrete:
      assert_true(_input);
      return utils::Partitioner<
          enums::DataUsed::same_units_discrete>::partition(_split, *_input,
                                                           _output, _begin,
                                                           _end);
    ...
    case enums::DataUsed::same_units_discrete_is_nan:
      assert_true(_input);
      return utils::Partitioner<enums::DataUsed::same_units_discrete_is_nan>::
          partition(_split.column_input_, _split.column_, *_input, _output,
                    _begin, _end);
    ...
    case enums::DataUsed::same_units_numerical:
      assert_true(_input);
      return utils::Partitioner<
          enums::DataUsed::same_units_numerical>::partition(_split, *_input,
                                                            _output, _begin,
                                                            _end);
    ...
    case enums::DataUsed::subfeatures:
      assert_true(_input);
      return utils::Partitioner<enums::DataUsed::subfeatures>::partition(
          _split, _subfeatures, _begin, _end);

    case enums::DataUsed::text_input:
      assert_true(_input);
      return utils::Partitioner<enums::DataUsed::text_input>::partition(
          _split, *_input, _begin, _end);

    case enums::DataUsed::text_output:
      assert_true(_input);
      return utils::Partitioner<enums::DataUsed::text_output>::partition(
          _split, _output, _begin, _end);

    case enums::DataUsed::time_stamps_window:
      assert_true(_input);
      return utils::Partitioner<enums::DataUsed::time_stamps_window>::partition(
          _split, hyperparameters().delta_t_, *_input, _output, _begin, _end);
...
void DecisionTreeNode::to_sql(
    ...
    const std::string& _sql, std::vector<std::string>* _conditions) const {
  if (child_greater_) {
    assert_true(child_smaller_);
    ...
    const auto sql_greater =
        ...
        condition_maker_.condition_greater(
            _categories, _vocab_popul, _vocab_perip, _sql_dialect_generator,
            _feature_prefix, input(), output(), split_);

    child_greater_->to_sql(_categories, _vocab_popul, _vocab_perip,
                           _sql_dialect_generator, _feature_prefix,
                           _feature_num, sql_greater, _conditions);
    ...
    child_smaller_->to_sql(_categories, _vocab_popul, _vocab_perip,
                           _sql_dialect_generator, _feature_prefix,
                           _feature_num, sql_smaller, _conditions);
  } else {
    const auto condition =
        _sql + " THEN " +
        condition_maker_.make_equation(_sql_dialect_generator, _feature_prefix,
                                       input(), output(), weights_);
...
Float DecisionTreeNode::transform(
  ...
  if (is_greater) {
    return child_greater_->transform(_output, _input, _subfeatures,
                                     _output_rescaled, _input_rescaled, _match);
  } else {
    return child_smaller_->transform(_output, _input, _subfeatures,
                                     _output_rescaled, _input_rescaled, _match);
  }
...
std::vector<containers::CandidateSplit> DecisionTreeNode::try_all(
    const Float _old_intercept, const containers::DataFrameView& _output,
  ...
  try_categorical_input(_old_intercept, _input, _output_rescaled,
                        _input_rescaled, _begin, _end, &bins, &candidates);

  try_discrete_input(_old_intercept, _input, _begin, _end, &bins, &candidates);

  try_numerical_input(_old_intercept, _input, _begin, _end, &bins, &candidates);

  try_same_units_categorical(_old_intercept, _input, _output, _begin, _end,
                             &candidates);

  try_same_units_discrete(_old_intercept, _input, _output, _begin, _end, &bins,
                          &candidates);

  try_same_units_numerical(_old_intercept, _input, _output, _begin, _end, &bins,
                           &candidates);

  try_text_input(_old_intercept, _input, _output_rescaled, _input_rescaled,
                 _begin, _end, &bins, &candidates);

  try_time_stamps_window(_old_intercept, _input, _output, _begin, _end, &bins,
                         &candidates);

  try_categorical_output(_old_intercept, _output, _output_rescaled,
                         _input_rescaled, _begin, _end, &bins, &candidates);

  try_discrete_output(_old_intercept, _output, _begin, _end, &bins,
                      &candidates);

  try_numerical_output(_old_intercept, _output, _begin, _end, &bins,
                       &candidates);

  try_text_output(_old_intercept, _output, _output_rescaled, _input_rescaled,
                  _begin, _end, &bins, &candidates);
...
void DecisionTreeNode::try_categorical_or_text(
    const enums::Revert _revert, const Int _min,
    const std::shared_ptr<const std::vector<Int>> _critical_values,
    const size_t _num_column, const Float _old_intercept,
  ...
  for (size_t i = 0; i < _critical_values->size(); ++i) {
    ...
    assert_true(static_cast<size_t>(cv - _min) < _indptr.size() - 1);

    const auto split_begin = _bins->begin() + _indptr[cv - _min];

    const auto split_end = _bins->begin() + _indptr[cv - _min + 1];

    add_candidates(_revert, update, _old_intercept,
                   containers::Split(_critical_values, critical_values_begin,
                                     _critical_values->begin() + i + 1,
                                     _num_column, _data_used),
                   _bins->begin(), split_begin, split_end, _bins->end(),
                   _candidates);
  }
...
void DecisionTreeNode::try_categorical_input(
    const Float _old_intercept, const containers::DataFrame& _input,
    const containers::Rescaled& _output_rescaled,
    const containers::Rescaled& _input_rescaled,
    const std::vector<containers::Match>::iterator _begin,
    const std::vector<containers::Match>::iterator _end,
  ...
  for (size_t j = 0; j < _input.num_categoricals(); ++j) {
    ...
    const auto [indptr, critical_values] =
        utils::CategoricalBinner<decltype(get_value)>::bin(
            min, max, get_value, _begin, nan_begin, _end, _bins, &comm());
    ...
    try_categorical_or_text(enums::Revert::True, min, critical_values, j,
                            _old_intercept, enums::DataUsed::categorical_input,
                            indptr, _bins, _candidates);

    // ----------------------------------------------------------------
    // Sort critical values by their associated weights in DESCENDING
    // order.

    const auto sorted_critical_values = utils::CriticalValueSorter::sort(
        min, std::nullopt, indptr, _output_rescaled, _input_rescaled,
        _candidates->begin() + begin_ix, _candidates->end(), _bins, comm_);

    // ----------------------------------------------------------------
    // Try combined categorical values.

    try_categorical_or_text(
        enums::Revert::False, min, sorted_critical_values, j, _old_intercept,
        enums::DataUsed::categorical_input, indptr, _bins, _candidates);
...
void DecisionTreeNode::try_categorical_output(
  ...
  for (size_t j = 0; j < _output.num_categoricals(); ++j) {
    ...
    try_categorical_or_text(enums::Revert::True, min, critical_values, j,
                            _old_intercept, enums::DataUsed::categorical_output,
                            indptr, _bins, _candidates);
    ...
    try_categorical_or_text(
        enums::Revert::False, min, sorted_critical_values, j, _old_intercept,
        enums::DataUsed::categorical_output, indptr, _bins, _candidates);
...
void DecisionTreeNode::try_discrete_input(
  ...
  for (size_t j = 0; j < _input.num_discretes(); ++j) {
    ...
    const auto nan_begin =
        utils::Partitioner<enums::DataUsed::discrete_input_is_nan>::partition(
    ...
    const auto [indptr, step_size] =
        utils::DiscreteBinner<decltype(get_value)>::bin(
            min, max, get_value, num_bins_numerical, _begin, nan_begin, _end,
            _bins);
    ...
    try_numerical_or_discrete(enums::DataUsed::discrete_input, j, 0,
                              _old_intercept, max, step_size, indptr, _bins,
                              _candidates);
  }
}
...
void DecisionTreeNode::try_discrete_output(
  ...
  for (size_t j = 0; j < _output.num_discretes(); ++j) {
    ...
    // very end.
    const auto nan_begin =
        utils::Partitioner<enums::DataUsed::discrete_output_is_nan>::partition(
    ...
    try_numerical_or_discrete(enums::DataUsed::discrete_output, j, 0,
                              _old_intercept, max, step_size, indptr, _bins,
                              _candidates);
  }
}
...
void DecisionTreeNode::try_numerical_or_discrete(
    const enums::DataUsed _data_used, const size_t _col1, const size_t _col2,
    const Float _old_intercept, const Float _max, const Float _step_size,
  ...
  for (size_t i = 1; i < _indptr.size(); ++i) {
    ...
    const auto critical_value = _max - static_cast<Float>(i) * _step_size;

    const auto split =
        is_same_units(_data_used)
            ? containers::Split(_col1, _col2, critical_value, _data_used)
            : containers::Split(_col1, critical_value, _data_used);

    add_candidates(enums::Revert::False, update, _old_intercept, split,
                   _bins->begin(), split_begin, split_end, _bins->end(),
                   _candidates);
  }
...
void DecisionTreeNode::try_numerical_input(
  ...
  for (size_t j = 0; j < _input.num_numericals(); ++j) {
    ...
    // very end.
    const auto nan_begin =
        utils::Partitioner<enums::DataUsed::numerical_input_is_nan>::partition(
            j, _input, _begin, _end);
    ...
    const auto [indptr, step_size] =
        utils::NumericalBinner<decltype(get_value)>::bin(
            min, max, get_value, num_bins, _begin, nan_begin, _end, _bins);

    if (indptr.size() == 0) {
      continue;
    }

    try_numerical_or_discrete(enums::DataUsed::numerical_input, j, 0,
                              _old_intercept, max, step_size, indptr, _bins,
                              _candidates);
  }
}
...
void DecisionTreeNode::try_numerical_output(
  ...
  for (size_t j = 0; j < _output.num_numericals(); ++j) {
    ...
    // very end.
    const auto nan_begin =
        utils::Partitioner<enums::DataUsed::numerical_output_is_nan>::partition(
            j, _output, _begin, _end);
    ...
    try_numerical_or_discrete(enums::DataUsed::numerical_output, j, 0,
                              _old_intercept, max, step_size, indptr, _bins,
                              _candidates);
  }
}
...
void DecisionTreeNode::try_same_units_categorical(
  ...
  for (size_t output_col = 0; output_col < _output.num_categoricals();
    ...
    for (size_t input_col = 0; input_col < _input.num_categoricals();
      ...
      const auto partition_function = [input_col, output_col, &_input,
                                       &_output](containers::Match m) {
        assert_true(m.ix_input < _input.nrows());
        assert_true(m.ix_output < _output.nrows());

        const bool is_same = (_input.categorical(m.ix_input, input_col) ==
                              _output.categorical(m.ix_output, output_col));

        return is_same;
      };

      const auto it = std::partition(_begin, _end, partition_function);

      add_candidates(enums::Revert::False, enums::Update::calc_all,
                     _old_intercept, containers::Split(output_col, input_col),
                     _begin, _begin, it, _end, _candidates);
  ...
  for (size_t output_col = 0; output_col < _output.num_discretes();
    ...
    for (size_t input_col = 0; input_col < _input.num_discretes();
      ...
      const auto nan_begin = utils::Partitioner<
          enums::DataUsed::same_units_discrete_is_nan>::partition(input_col,
                                                                  output_col,
                                                                  _input,
                                                                  _output,
                                                                  _begin, _end);

      const auto get_value = [input_col, output_col, &_input,
                              &_output](const containers::Match& m) {
      ...
      const bool is_ts =
          _output.discrete_unit(output_col).find("time stamp") !=
              std::string::npos &&
          _output.discrete_name(output_col).find("$GETML_ROWID") ==
              std::string::npos;

      const auto data_used = is_ts ? enums::DataUsed::same_units_discrete_ts
                                   : enums::DataUsed::same_units_discrete;

      try_numerical_or_discrete(data_used, output_col, input_col,
                                _old_intercept, max, step_size, indptr, _bins,
                                _candidates);
    }
  }
...
void DecisionTreeNode::try_same_units_numerical(
  ...
  for (size_t output_col = 0; output_col < _output.num_numericals();
    ...
    for (size_t input_col = 0; input_col < _input.num_numericals();
      ...
      // the very end.
      const auto nan_begin =
          utils::Partitioner<enums::DataUsed::same_units_numerical_is_nan>::
              partition(input_col, output_col, _input, _output, _begin, _end);
      ...
      const bool is_ts =
          _output.numerical_unit(output_col).find("time stamp") !=
              std::string::npos &&
          _output.numerical_name(output_col).find(helpers::Macros::rowid()) ==
              std::string::npos;

      const auto data_used = is_ts ? enums::DataUsed::same_units_numerical_ts
...
void DecisionTreeNode::try_subfeatures(
    const Float _old_intercept, const containers::Subfeatures& _subfeatures,
    const std::vector<containers::Match>::iterator _begin,
    const std::vector<containers::Match>::iterator _end,
    std::vector<containers::Match>* _bins,
    std::vector<containers::CandidateSplit>* _candidates) {
  for (size_t j = 0; j < _subfeatures.size(); ++j) {
    assert_true(std::all_of(
        ...
        [](const Float val) { return !std::isnan(val) && !std::isinf(val); }));

    const auto get_value = [j, &_subfeatures](const containers::Match& m) {
      const auto i = m.ix_input;
    ...
    try_numerical_or_discrete(enums::DataUsed::subfeatures, j, 0,
                              _old_intercept, max, step_size, indptr, _bins,
                              _candidates);
  }
}
...
void DecisionTreeNode::try_text(
  ...
  add_candidates(enums::Revert::True, enums::Update::calc_all, _old_intercept,
                 containers::Split(_words, _words->begin(), _words->begin(),
                                   _num_column, _data_used),
                 _bins->begin(), _bins->begin(), _bins->begin(), _bins->end(),
                 &fake_candidates);
  ...
  for (size_t i = 0; i < _words->size(); ++i) {
    ...
    textmining::Matches::extract(word, _row_index, _indptr, _bins->begin(),
                                 _bins->end(), &extracted);

    add_candidates(
        enums::Revert::True, enums::Update::calc_diff, _old_intercept,
        containers::Split(_words, _words->begin() + i, _words->begin() + i + 1,
                          _num_column, _data_used),
        extracted.begin(), extracted.begin(), extracted.end(), extracted.end(),
        _candidates);
  }
...
void DecisionTreeNode::try_text_input(
  ...
  for (size_t j = 0; j < _input.num_text(); ++j) {
    ...
    const auto rownum_indptr = try_text_input_single_word(
        j, _old_intercept, _input, _begin, _end, _bins, _candidates);

    // ----------------------------------------------------------------

    try_text_input_multiple_words(j, begin_ix, _old_intercept, rownum_indptr,
                                  _input, _output_rescaled, _input_rescaled,
                                  _begin, _end, _bins, _candidates);
...
std::vector<size_t> DecisionTreeNode::try_text_input_single_word(
    const size_t _num_column, const Float _old_intercept,
    const containers::DataFrame& _input,
    const std::vector<containers::Match>::iterator _begin,
    const std::vector<containers::Match>::iterator _end,
  ...
  const auto get_rownum = [](const containers::Match& m) { return m.ix_input; };
  ...
  try_text(words, _num_column, _old_intercept, enums::DataUsed::text_input,
           *row_index, rownum_indptr, _bins, _candidates);

  // ----------------------------------------------------------------

  return rownum_indptr;
...
void DecisionTreeNode::try_text_input_multiple_words(
    const size_t _num_column, const size_t _begin_ix,
    const Float _old_intercept, const std::vector<size_t>& _rownum_indptr,
    const containers::DataFrame& _input,
    const containers::Rescaled& _output_rescaled,
    const containers::Rescaled& _input_rescaled,
    const std::vector<containers::Match>::iterator _begin,
  ...
  assert_true(_num_column < _input.row_indices_.size());
  ...
  const auto sorted_words = utils::CriticalValueSorter::sort(
      0, *row_index, _rownum_indptr, _output_rescaled, _input_rescaled,
      _candidates->begin() + _begin_ix, _candidates->end(), _bins, comm_);
  ...
  const auto extract_word = [&word_index,
                             sorted_words](const containers::Match& m) -> Int {
  ...
  try_categorical_or_text(enums::Revert::False,
                          ...
                          enums::DataUsed::text_input, word_indptr, _bins,
                          _candidates);
...
void DecisionTreeNode::try_text_output(
  ...
  for (size_t j = 0; j < _output.num_text(); ++j) {
    ...
    const auto rownum_indptr = try_text_output_single_word(
        j, _old_intercept, _output, _begin, _end, _bins, _candidates);

    // ----------------------------------------------------------------

    try_text_output_multiple_words(j, begin_ix, _old_intercept, rownum_indptr,
                                   _output, _output_rescaled, _input_rescaled,
                                   _begin, _end, _bins, _candidates);
...
std::vector<size_t> DecisionTreeNode::try_text_output_single_word(
    const size_t _num_column, const Float _old_intercept,
    const containers::DataFrameView& _output,
    const std::vector<containers::Match>::iterator _begin,
    const std::vector<containers::Match>::iterator _end,
...
void DecisionTreeNode::try_text_output_multiple_words(
  ...
  assert_true(_num_column < df.row_indices_.size());
...
void DecisionTreeNode::try_time_stamps_window(
  ...
  for (size_t i = 1; i < indptr.size(); ++i) {
    ...
    const auto critical_value = max - static_cast<Float>(i) * step_size;

    add_candidates(enums::Revert::True, update, _old_intercept,
                   containers::Split(0, critical_value,
