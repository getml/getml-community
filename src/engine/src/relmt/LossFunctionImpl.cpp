...
void LossFunctionImpl::calc_g_ptr(const std::vector<size_t>& _indices_current,
  ...
  for (const auto ix : _indices_current) {
    ...
    for (size_t j = 0; j < ncolsplus1; ++j) {
      const auto d_eta =
          _eta1[ix * ncolsplus1 + j] - _eta1_old[ix * ncolsplus1 + j];
    ...
    for (size_t j = 0; j < ncolsplus1; ++j) {
      const auto d_eta =
          _eta2[ix * ncolsplus1 + j] - _eta2_old[ix * ncolsplus1 + j];
...
void LossFunctionImpl::calc_Hfwf_ptr(
  ...
  for (const auto ix : _indices_current) {
    ...
    const auto w_old = calc_w_old(_old_weights, &_eta1[ncolsplus1 * ix],
                                  &_eta2[ncolsplus1 * ix]);
    ...
    update_Hfwf(ncolsplus1, h_sample_weights, w_fixed, &_eta1[ncolsplus1 * ix],
                &_eta1_old[ncolsplus1 * ix], &_eta2[ncolsplus1 * ix],
                &_eta2_old[ncolsplus1 * ix], _Hfwf_ptr);
  }
}

// ----------------------------------------------------------------------------

void LossFunctionImpl::calc_H_ptr(const enums::Update _update,
                                  const std::vector<size_t>& _indices_current,
                                  const std::vector<Float>& _eta1,
                                  const std::vector<Float>& _eta1_old,
                                  const std::vector<Float>& _eta2,
  ...
  if (_update != enums::Update::calc_diff) {
    ...
    for (size_t j = 1; j < dim; ++j) {
      _H_ptr[j * (j + 1) / 2 + j] = hyperparameters().reg_lambda_;
    }
  }
  ...
  for (const auto ix : _indices_current) {
    ...
    if (_update != enums::Update::calc_one) {
      for (size_t j = 0; j < ncolsplus1; ++j) {
        old_vals[j + ncolsplus1 + 1] = _eta2_old[ix * ncolsplus1 + j];
        new_vals[j + ncolsplus1 + 1] = _eta2[ix * ncolsplus1 + j];
      }
    }

    assert_true(std::all_of(
        old_vals.begin(), old_vals.end(),
        [](const Float val) { return !std::isnan(val) && !std::isinf(val); }));

    assert_true(std::all_of(
        new_vals.begin(), new_vals.end(),
        [](const Float val) { return !std::isnan(val) && !std::isinf(val); }));
    ...
    for (size_t j = 0; j < dim; ++j) {
      for (size_t k = 0; k <= j; ++k) {
        _H_ptr[j * (j + 1) / 2 + k] +=
            h_sample_weights *
            (new_vals[j] * new_vals[k] - old_vals[j] * old_vals[k]);
      }
    }
  }
}
...
std::pair<Float, containers::Weights> LossFunctionImpl::calc_pair(
  ...
  calc_sufficient_stats(_update, _old_weights, _indices_current, _eta1,
  ...
  utils::Reducer::reduce<Float>(std::plus<Float>(), &sufficient_stats_global,
                                _comm);
  ...
  auto weights_tuple =
      std::make_tuple(intercept, std::move(weights1), std::move(weights2));
...
void LossFunctionImpl::calc_sufficient_stats(
  ...
  if (_update != enums::Update::calc_one) {
    calc_Hfwf_ptr(_update, _old_weights, _indices_current, _eta1, _eta1_old,
                  _eta2, _eta2_old, _yhat_committed, Hfwf_ptr);
  }

  calc_H_ptr(_update, _indices_current, _eta1, _eta1_old, _eta2, _eta2_old,
             H_ptr);
...
std::pair<Float, Eigen::VectorXd> LossFunctionImpl::calc_results(
  ...
  for (int i = 0; i < dim; ++i) {
    assert_true(!std::isnan(Hfwf_ptr[i]) && !std::isinf(Hfwf_ptr[i]));
    Hfwf(i) = Hfwf_ptr[i];
  }
  ...
  for (int i = 0; i < dim; ++i) {
    for (int j = 0; j < i; ++j) {
      ...
      assert_true(!std::isnan(H(i, j)) && !std::isinf(H(i, j)));
    }
...
void LossFunctionImpl::calc_yhat(const std::vector<Float>& _old_weights,
