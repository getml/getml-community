...
{
  update_etas(_divide_by_count, _indices_current, _eta1, _eta1_old, _eta2,
              _eta2_old, &eta1_, &eta2_);
...
std::pair<Float, containers::Weights> IntermediateAggregationImpl::calc_pair(
    ...
    const std::vector<Float>& _old_weights, const std::vector<size_t>& _indices,
    const std::vector<size_t>& _indices_current,
    const std::vector<Float>& _eta1, const std::vector<Float>& _eta1_old,
    const std::vector<Float>& _eta2, const std::vector<Float>& _eta2_old) {
  ...
  const auto [eta1, eta1_old, eta2, eta2_old] =
      calc_etas(_divide_by_count, _agg, _indices_current, _eta1, _eta1_old,
                _eta2, _eta2_old);

  const auto result =
      child_->calc_pair(_agg, _revert, _update, _old_weights, indices(),
                        indices_current(), *eta1, *eta1_old, *eta2, *eta2_old);
...
IntermediateAggregationImpl::reduce_predictions(
  ...
  auto predictions = std::make_shared<std::vector<Float>>(agg_index().nrows());
  ...
  for (size_t i = 0; i < _input_predictions.size(); ++i) {
    ...
    for (const auto ix : indices) {
      assert_true(ix >= 0);
      assert_true(static_cast<size_t>(ix) < predictions->size());
...
void IntermediateAggregationImpl::update_etas(
    const bool _divide_by_count, const std::vector<size_t>& _indices_current,
    const std::vector<Float>& _eta1_input,
    const std::vector<Float>& _eta1_input_old,
    const std::vector<Float>& _eta2_input,
    const std::vector<Float>& _eta2_input_old, std::vector<Float>* _eta1_output,
    std::vector<Float>* _eta2_output) {
  indices_current_.clear();

  if (_divide_by_count) {
    update_etas_divide_by_count(_indices_current, _eta1_input, _eta1_input_old,
                                _eta2_input, _eta2_input_old, _eta1_output,
                                _eta2_output);
  } else {
    update_etas_dont_divide(_indices_current, _eta1_input, _eta1_input_old,
                            _eta2_input, _eta2_input_old, _eta1_output,
                            _eta2_output);
  }
}
  ...
  for (auto ix_input : _indices_current) {
    ...
    for (auto ix_output : output_indices) {
      ...
      for (size_t j = 0; j < ncols(); ++j) {
        ...
        (*_eta1_output)[ix2] +=
            (_eta1_input[ix1] - _eta1_input_old[ix1]) / count;
      }
      ...
      for (size_t j = 0; j < ncols(); ++j) {
        ...
        (*_eta2_output)[ix2] +=
            (_eta2_input[ix1] - _eta2_input_old[ix1]) / count;
      }
  ...
  for (auto ix_input : _indices_current) {
    ...
    for (auto ix_output : output_indices) {
      ...
      for (size_t j = 0; j < ncols(); ++j) {
        ...
        (*_eta1_output)[ix2] += (_eta1_input[ix1] - _eta1_input_old[ix1]);
      }
      ...
      for (size_t j = 0; j < ncols(); ++j) {
        ...
        (*_eta2_output)[ix2] += (_eta2_input[ix1] - _eta2_input_old[ix1]);
      }
