...

{
  ...
  init_predictors("feature_selectors_", 1, impl_.feature_selector_impl_,
                  fl_fingerprints());

  init_predictors("predictors_", 1, impl_.predictor_impl_, fl_fingerprints());
}

// ----------------------------------------------------------------------------

Pipeline::Pipeline(const std::string& _path,
...
void Pipeline::add_population_cols(
  ...
  for (const auto& col : _predictor_impl.numerical_colnames()) {
    _features->push_back(helpers::Feature<Float>(
        _population_df.numerical(col).clone().data_ptr()));
  }
}
...
void Pipeline::calculate_feature_stats(
    const containers::NumericalFeatures _features, const size_t _nrows,
...
void Pipeline::check(const CheckParams& _params) const {
  // -------------------------------------------------------------------------

  assert_true(_params.categories_);

  auto [population_df, peripheral_dfs] = modify_data_frames(
      _params.population_df_, _params.peripheral_dfs_, _params.logger_,
      _params.categories_->temp_dir(), _params.socket_);

  const auto df_fingerprints =
      extract_df_fingerprints(_params.population_df_, _params.peripheral_dfs_);
  ...
  const auto fl_fingerprints = feature_learners.size() > 0
  ...
  fit_transform_preprocessors(_params.cmd_, _params.logger_,
                              _params.categories_,
                              _params.preprocessor_tracker_, df_fingerprints,
                              &population_df, &peripheral_dfs, _params.socket_);
  ...
  const auto warner = preprocessors::DataModelChecker::check(
      placeholder, peripheral_names, population_df, peripheral_dfs,
      feature_learners, socket_logger);
...
void Pipeline::column_importances_manual(
  ...
  for (size_t i = 0; i < _f_importances.size(); ++i) {
    ...
    assert_true(j + predictor_impl().num_manual_features() ==
    ...
    for (const auto& colname : predictor_impl().categorical_colnames()) {
      const auto desc = helpers::ColumnDescription(
          _importance_makers->at(i).population(), *population_name, colname);
...
std::vector<Poco::JSON::Object::Ptr> Pipeline::extract_fs_fingerprints() const {
  if (feature_selectors_.size() == 0 || feature_selectors_.at(0).size() == 0) {
    return fl_fingerprints();
  }
...
Pipeline::extract_modified_schemata(
  ...
  const auto population_schema =
      std::make_shared<const helpers::Schema>(extract_schema(_population_df));
...
Poco::JSON::Object Pipeline::feature_importances_as_obj() const {
  ...
  const auto feature_importances_transposed = feature_importances(predictors_);
...
std::vector<std::string> Pipeline::feature_learners_to_sql(
  ...
  const auto to_sql =
      [this, _categories, _targets, _subfeatures,
       _sql_dialect_generator](const size_t _i) -> std::vector<std::string> {
    const auto& fl = feature_learners_.at(_i);

    assert_true(fl);

    const auto all =
        fl->to_sql(_categories, _targets, _subfeatures, _sql_dialect_generator,
...
Pipeline::feature_names() const {
  ...
  const auto numerical =
      helpers::Macros::modify_colnames(predictor_impl().numerical_colnames());

  const auto categorical =
      helpers::Macros::modify_colnames(predictor_impl().categorical_colnames());
...
containers::NumericalFeatures Pipeline::generate_autofeatures(
...
containers::NumericalFeatures Pipeline::generate_predictions(
  ...
  for (size_t i = 0; i < num_predictor_sets(); ++i) {
    auto mean_prediction = helpers::Feature<Float>(
        std::make_shared<std::vector<Float>>(nrows));  // TODO: Use actual pool
...
void Pipeline::fit(const FitParams& _params) {
  ...
  preprocessors_ = fit_transform_preprocessors(
      _params.cmd_, _params.logger_, _params.categories_,
      _params.preprocessor_tracker_, df_fingerprints(), &population_df,
      &peripheral_dfs, _params.socket_);
  ...
  fit_feature_learners(_params, population_df, peripheral_dfs);
  ...
  auto feature_selectors =
      init_predictors("feature_selectors_", num_targets(),
                      impl_.feature_selector_impl_, fl_fingerprints());

  const auto feature_selection_params =
      TransformParams{.categories_ = _params.categories_,
                      .cmd_ = _params.cmd_,
  ...
  auto predictors = init_predictors("predictors_", num_targets(),
                                    impl_.predictor_impl_, dependencies);

  const auto prediction_params =
      TransformParams{.categories_ = _params.categories_,
                      .cmd_ = _params.cmd_,
                      .data_frames_ = _params.data_frames_,
                      .data_frame_tracker_ = _params.data_frame_tracker_,
...
void Pipeline::fit_feature_learners(
    ...
    const std::vector<containers::DataFrame>& _peripheral_dfs) {
  std::tie(impl_.modified_population_schema_,
           impl_.modified_peripheral_schema_) =
...
void Pipeline::fit_predictors(const TransformParams& _params) {
  ...
  *_params.autofeatures_ = autofeatures;
  ...
  categorical_features =
      _params.predictor_impl_.value().transform_encodings(categorical_features);
  ...
  if (categorical_features_valid) {
    *categorical_features_valid =
        _params.predictor_impl_.value().transform_encodings(
            *categorical_features_valid);
  }
  ...
  assert_true(_params.population_df_);
  assert_true(_params.population_df_.value().num_targets() ==
              _params.predictors_->size());

  // --------------------------------------------------------------------

  for (size_t t = 0; t < _params.population_df_.value().num_targets(); ++t) {
    const auto target_col = helpers::Feature<Float>(
        _params.population_df_.value().target(t).data_ptr());

    const auto target_col_valid =
        numerical_features_valid
            ? std::make_optional<decltype(target_col)>(
                  _params.validation_df_.value().target(t).to_vector_ptr())
...
containers::NumericalFeatures Pipeline::get_numerical_features(
  ...
  add_population_cols(_cmd, _population_df, _predictor_impl,
                      &numerical_features);
...
std::vector<std::string> Pipeline::get_targets(
  ...
  auto target_names = std::vector<std::string>(_population_df.num_targets());
...
Pipeline::init_feature_learners(
  ...
  const auto to_fl = [this, &_dependencies, placeholder = placeholder,
...
Pipeline::init_preprocessors(
  ...
  const auto arr = jsonutils::JSON::get_object_array(obj(), "preprocessors_");
...
bool Pipeline::is_classification() const {
  ...
  bool all_classifiers =
      std::all_of(feature_learners_.begin(), feature_learners_.end(), fl_is_cl);

  for (const auto& fs : feature_selectors_) {
    all_classifiers =
        all_classifiers && std::all_of(fs.begin(), fs.end(), pred_is_cl);
  }

  for (const auto& p : predictors_) {
    all_classifiers =
        all_classifiers && std::all_of(p.begin(), p.end(), pred_is_cl);
  }

  // -----------------------------------------------------------------------

  bool all_regressors = std::none_of(feature_learners_.begin(),
                                     feature_learners_.end(), fl_is_cl);

  for (const auto& fs : feature_selectors_) {
    all_regressors =
        all_regressors && std::none_of(fs.begin(), fs.end(), pred_is_cl);
  }

  for (const auto& p : predictors_) {
    all_regressors =
        all_regressors && std::none_of(p.begin(), p.end(), pred_is_cl);
  }
...
Pipeline Pipeline::load(
  ...
  const auto scores = metrics::Scores(load_json_obj(_path + "scores.json"));
...
void Pipeline::load_feature_selectors(
  ...
  _pipeline->feature_selectors_ = _pipeline->init_predictors(
      "feature_selectors_", _pipeline->num_targets(),
      _pipeline->impl_.feature_selector_impl_, _pipeline->fl_fingerprints());

  assert_true(_pipeline->num_targets() == _pipeline->feature_selectors_.size());

  for (size_t i = 0; i < _pipeline->num_targets(); ++i) {
    for (size_t j = 0; j < _pipeline->feature_selectors_.at(i).size(); ++j) {
...
void Pipeline::load_fingerprints(const Poco::JSON::Object& _pipeline_json,
                                 Pipeline* _pipeline) const {
...
void Pipeline::load_pipeline_json(const std::string& _path,
                                  Pipeline* _pipeline) const {
  ...
  _pipeline->allow_http() = JSON::get_value<bool>(pipeline_json, "allow_http_");
  ...
  _pipeline->peripheral_schema() = get_peripheral_schema("peripheral_schema_");
...
void Pipeline::load_predictors(
  ...
  _pipeline->predictors_ = _pipeline->init_predictors(
      "predictors_", _pipeline->num_targets(), _pipeline->impl_.predictor_impl_,
...
containers::NumericalFeatures Pipeline::make_autofeatures(
    const TransformParams& _params) const {
  assert_true(_params.predictor_impl_);
  ...
  if (_params.autofeatures_ && _params.autofeatures_->size() != 0) {
    return select_autofeatures(*_params.autofeatures_,
                               _params.predictor_impl_.value());
  }

  return generate_autofeatures(_params);
}

// ----------------------------------------------------------------------------

std::vector<std::string> Pipeline::make_feature_names() const {
  const auto to_names = [this](const size_t _i) -> std::vector<std::string> {
...
Pipeline::make_features(const TransformParams& _params) const {
  ...
  const auto df = _params.data_frame_tracker_.retrieve(
      dependencies(), _params.original_population_df_.value(),
...
void Pipeline::make_feature_selector_impl(
  ...
  if (impl_.include_categorical_) {
    for (size_t i = 0; i < _population_df.num_categoricals(); ++i) {
      if (_population_df.categorical(i).unit().find("comparison only") !=
          std::string::npos) {
        continue;
      }

      if (helpers::SubroleParser::contains_any(
              _population_df.categorical(i).subroles(), blacklist)) {
        continue;
      }

      categorical_colnames.push_back(_population_df.categorical(i).name());
    }
  }
  ...
  for (size_t i = 0; i < _population_df.num_numericals(); ++i) {
    if (_population_df.numerical(i).unit().find("comparison only") !=
        std::string::npos) {
      continue;
    }
    ...
    const auto contains_null =
        std::any_of(_population_df.numerical(i).begin(),
                    _population_df.numerical(i).end(), is_null);

    if (contains_null) {
      continue;
    }

    numerical_colnames.push_back(_population_df.numerical(i).name());
  }
...
void Pipeline::move_tfile(const std::string& _path, const std::string& _name,
...
void Pipeline::save(const helpers::StringIterator& _categories,
  ...
  feature_selector_impl().save(tfile.path() + "/feature-selector-impl.json");
...
void Pipeline::save_pipeline_json(const Poco::TemporaryFile& _tfile) const {
  ...
  pipeline_json.set("df_fingerprints_",
                    JSON::vector_to_array(df_fingerprints()));

  pipeline_json.set("fl_fingerprints_",
                    JSON::vector_to_array(fl_fingerprints()));
...
Poco::JSON::Object Pipeline::score(const containers::DataFrame& _population_df,
                                   const std::string& _population_name,
                                   const containers::NumericalFeatures& _yhat) {
  // ------------------------------------------------

  containers::NumericalFeatures y;
...
void Pipeline::score_after_fitting(const TransformParams& _params) {
  auto [numerical_features, categorical_features, _] = make_features(_params);
  ...
  score(_params.population_df_.value(), name, yhat);
}

// ----------------------------------------------------------------------------

containers::NumericalFeatures Pipeline::select_autofeatures(
    const containers::NumericalFeatures& _autofeatures,
    const predictors::PredictorImpl& _predictor_impl) const {
  // ------------------------------------------------
...
std::vector<std::string> Pipeline::staging_to_sql(
  ...
  const auto population_needs_targets =
      _targets | std::any_of(feature_learners_.begin(), feature_learners_.end(),
                             needs_targets);
...
std::string Pipeline::to_sql(const helpers::StringIterator& _categories,
  ...
  const auto sql_dialect_generator = helpers::SQLDialectParser::parse(_dialect);
  ...
  const auto preprocessing =
      _full_pipeline ? preprocessors_to_sql(_categories, sql_dialect_generator)
                     : std::vector<std::string>();
  ...
  return sql_dialect_generator->make_sql(
      modified_population_schema()->name_, autofeatures, sql, target_names,
...
Pipeline::transform(const TransformParams& _params) {
  // -------------------------------------------------------------------------
  ...
  auto [population_df, peripheral_dfs] = modify_data_frames(
      ...
      _params.original_peripheral_dfs_.value(), _params.logger_,
      _params.categories_->temp_dir(), _params.socket_);

  // -------------------------------------------------------------------------

  std::tie(population_df, peripheral_dfs) =
      apply_preprocessors(_params.cmd_, _params.logger_, population_df,
                          peripheral_dfs, _params.categories_, _params.socket_);
  ...
  const auto params = TransformParams{
      ...
      .original_population_df_ = _params.original_population_df_,
      .peripheral_dfs_ = peripheral_dfs,
      .population_df_ = population_df,
      .predictor_impl_ = predictor_impl(),
  ...
  if (score && ncols > 0) {
    ...
    calculate_feature_stats(numerical_features, nrows, ncols, _params.cmd_,
                            population_df);
  }
