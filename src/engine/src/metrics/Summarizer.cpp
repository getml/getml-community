include "metrics/Summarizer.hpp"
  ...
  Poco::JSON::Object Summarizer::calculate_feature_plots(
    ...
    auto average_targets_arr = calculate_average_targets(
        minima, step_sizes, actual_num_bins, feature_densities, _features,
        _nrows, _ncols, _targets);
  ...
  void Summarizer::find_min_and_max(
      const Features& _features, const size_t _nrows, const size_t _ncols,
      std::vector<Float>* _minima, std::vector<Float>* _maxima) {
  ...
  size_t Summarizer::identify_bin(const size_t _num_bins,
                                  const Float _step_size, const Float _val,
                                  const Float _min) {
