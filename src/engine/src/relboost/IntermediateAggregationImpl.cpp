...

{
  if (_agg == enums::Aggregation::avg_first_null) {
    update_etas(_divide_by_count, _indices_current, _eta1, _eta1_old, _eta2,
                _eta2_old, &eta2_1_null_, &w_fixed_2_);

    return std::make_tuple(&eta2_1_null_, &eta2_1_null_old_, &w_fixed_2_,
                           &w_fixed_2_old_);
  } else if (_agg == enums::Aggregation::avg_second_null) {
    update_etas(_divide_by_count, _indices_current, _eta1, _eta1_old, _eta2,
                _eta2_old, &eta1_2_null_, &w_fixed_1_);

    return std::make_tuple(&eta1_2_null_, &eta1_2_null_old_, &w_fixed_1_,
                           &w_fixed_1_old_);
  } else {
    update_etas(_divide_by_count, _indices_current, _eta1, _eta1_old, _eta2,
