...
void SubtreeHelper::fit_subensemble(
    ...
    const Hyperparameters& _hyperparameters, const size_t _ix_perip_used,
  ...
  const auto input_map = utils::Mapper::create_rows_map(input_table.rows_ptr());

  const auto aggregation_index =
      std::make_shared<aggregations::AggregationIndex>(
          input_table, _table_holder->main_tables()[_ix_perip_used], input_map,
          _output_map);
  ...
  if (_agg_type == "AVG") {
    intermediate_agg = std::make_shared<aggregations::Avg>(
        aggregation_index, _loss_function,
        _table_holder->peripheral_tables()[_ix_perip_used],
        _table_holder->main_tables()[_ix_perip_used], _comm);
  ...
  (*_subensemble)->fit_subensembles(subtable_holder, _logger, intermediate_agg);

  auto predictions =
      (*_subensemble)->make_subpredictions(*subtable_holder, _logger, _comm);
  ...
  utils::Logger::log("Relboost: Training subfeatures...", _logger, _comm);

  while ((*_subensemble)->num_features() < num_features) {
    (*_subensemble)
        ->fit_new_features(intermediate_agg, subtable_holder, subfeatures,
                           num_features);
    ...
    utils::Logger::log(
        "Trained new features. Progress: " + std::to_string(progress) + "%.",
        _logger, _comm);
  }
...
void SubtreeHelper::fit_subensembles(
  ...
  const auto peripheral =
      std::make_shared<const std::vector<std::string>>(_ensemble.peripheral());
  ...
  for (size_t i = 0; i < num_tables; ++i) {
    if (_table_holder->subtables().at(i)) {
      ...
      const auto joined_table = std::make_shared<const containers::Placeholder>(
      ...
      const auto all_propositionalization =
          std::all_of(joined_table->propositionalization().begin(),
                      joined_table->propositionalization().end(), is_true);

      if (all_propositionalization) {
        continue;
      }

      subensembles_avg.at(i) = std::make_optional<DecisionTreeEnsemble>(
          hyperparameters, peripheral, joined_table);

      subensembles_sum.at(i) = std::make_optional<DecisionTreeEnsemble>(
          hyperparameters, peripheral, joined_table);
    }
  }
  ...
  const bool no_subfeatures =
      std::none_of(subensembles_avg.cbegin(), subensembles_avg.cend(),
  ...
  for (size_t i = 0; i < num_tables; ++i) {
    if (subensembles_avg.at(i)) {
      fit_subensemble("AVG", _table_holder, _logger, rows_map,
                      _ensemble.hyperparameters(), i, _loss_function, _comm,
  ...
  for (size_t i = 0; i < num_tables; ++i) {
    if (subensembles_sum.at(i)) {
      fit_subensemble("SUM", _table_holder, _logger, rows_map,
                      _ensemble.hyperparameters(), i, _loss_function, _comm,
...
std::vector<containers::Predictions> SubtreeHelper::make_predictions(
  ...
  for (size_t i = 0; i < size; ++i) {
    ...
    const auto all_propositionalization =
        std::all_of(subtable_holder.propositionalization().begin(),
                    subtable_holder.propositionalization().end(), is_true);
    ...
    make_predictions_for_one_subensemble(_subensembles_avg.at(i),
                                         subtable_holder, _logger, _comm,
                                         &predictions.at(i));

    make_predictions_for_one_subensemble(_subensembles_sum.at(i),
                                         subtable_holder, _logger, _comm,
                                         &predictions.at(i));
  }
...
void SubtreeHelper::make_predictions_for_one_subensemble(
  ...
  utils::Logger::log("Relboost: Building subfeatures...", _logger, _comm);

  const auto num_features = _subensemble->num_features();

  for (size_t i = 0; i < num_features; ++i) {
    _predictions->emplace_back(
        _subensemble->transform(_subtable_holder, subsubfeatures, i));

    assert_true(_predictions->back());

    assert_true(std::all_of(
        _predictions->back()->begin(), _predictions->back()->end(),
...
std::vector<containers::Subfeatures> SubtreeHelper::make_subfeatures(
  ...
  for (size_t i = 0; i < size; ++i) {
    ...
    assert_true(_table_holder.subtables().at(i)->main_tables().size() > 0);

    assert_true(
        _table_holder.subtables().at(i)->main_tables().at(0).rows_ptr());
    ...
    for (size_t j = 0; j < p.size(); ++j) {
      ...
      assert_true(
          std::all_of(p.at(j)->begin(), p.at(j)->end(), [](const Float val) {
      ...
      assert_true(
          std::all_of(column.begin(), column.end(), [](const Float val) {
            return !std::isnan(val) && !std::isinf(val);
          }));

      const auto view =
          containers::ColumnView<Float, std::map<Int, Int>>(column, rows_map);

      assert_true(std::all_of(view.col().begin(), view.col().end(),
                              [](const Float val) {
                                return !std::isnan(val) && !std::isinf(val);
                              }));
