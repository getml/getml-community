...
void Avg::activate(const containers::IntSet::Iterator _begin,
  ...
  for (auto it = _begin; it != _end; ++it) {
    ...
    if (count_committed_[ix] + count1_[ix] == 0.0) {
      ...
      eta1_2_null_[ix] = count1_[ix] / (count_committed_[ix] + count1_[ix]);

      w_fixed_1_[ix] = w_fixed_committed_[ix] * count_committed_[ix] /
    ...
    if (count_committed_[ix] + count2_[ix] == 0.0) {
      ...
      eta2_1_null_[ix] = count2_[ix] / (count_committed_[ix] + count2_[ix]);

      w_fixed_2_[ix] = w_fixed_committed_[ix] * count_committed_[ix] /
...
void Avg::calc_all(const enums::Revert _revert, const Float _old_weight,
...
void Avg::calc_diff(const Float _old_weight,
                    const std::vector<containers::Match>::iterator _split_begin,
                    const std::vector<containers::Match>::iterator _split_end) {
  // ------------------------------------------------------------------------
  ...
  if (allow_null_weights_) {
    if (std::isnan(_old_weight)) {
      ...
      deactivate(_old_weight, indices_current_.begin(), indices_current_.end());
    }
  }
...
void Avg::calc_etas(const enums::Aggregation _agg, const enums::Update _update,
  ...
  child_->calc_etas(_agg, _update, _old_weight,
...

{
  ...
  const auto weights = child_->calc_pair(
      _agg, _revert, _update, _old_weight, intermediate_agg().indices(),
      intermediate_agg().indices_current(), *eta1, *eta1_old, *eta2, *eta2_old);
...
std::vector<std::pair<Float, std::array<Float, 3>>> Avg::calc_pairs(
  ...
  switch (_update) {
    case enums::Update::calc_all:
      update_ = enums::Update::calc_all;
      calc_all(_revert, _old_weight, _begin, _split_begin, _split_end, _end);
      break;
  ...
  if (!impl_.is_balanced(num_samples_1_, num_samples_2_, _min_num_samples,
                         comm_)) {
    return results;
  }
  ...
  if (!std::isnan(_old_weight)) {
    results.push_back(child_->calc_pair(
        enums::Aggregation::avg, _revert, update_, _old_weight,
  ...
  if (allow_null_weights_) {
    results.push_back(child_->calc_pair(
        enums::Aggregation::avg_second_null, _revert, update_, _old_weight,
        indices_.unique_integers(), indices_current_.unique_integers(),
        eta1_2_null_, eta1_2_null_old_, w_fixed_1_, w_fixed_2_old_));

    results.push_back(child_->calc_pair(
        enums::Aggregation::avg_first_null, _revert, update_, _old_weight,
        indices_.unique_integers(), indices_current_.unique_integers(),
        eta2_1_null_, eta2_1_null_old_, w_fixed_2_, w_fixed_2_old_));
  }
...
void Avg::calc_yhat(const Float _old_weight,
  ...
  if (std::isnan(std::get<2>(_new_weights))) {
    ...
    child_->calc_yhat(enums::Aggregation::avg_second_null, _old_weight,
                      _new_weights, indices_.unique_integers(), eta1_2_null_,
                      eta1_2_null_old_, w_fixed_1_, w_fixed_1_old_);
  } else if (std::isnan(std::get<1>(_new_weights))) {
    assert_true(!std::isnan(std::get<2>(_new_weights)));

    child_->calc_yhat(enums::Aggregation::avg_first_null, _old_weight,
                      _new_weights, indices_.unique_integers(), eta2_1_null_,
                      eta2_1_null_old_, w_fixed_2_, w_fixed_2_old_);
  } else {
    child_->calc_yhat(enums::Aggregation::avg, _old_weight, _new_weights,
...
void Avg::calc_yhat(const enums::Aggregation _agg, const Float _old_weight,
                    const std::array<Float, 3>& _new_weights,
                    const std::vector<size_t>& _indices,
                    const std::vector<Float>& _eta1,
                    const std::vector<Float>& _eta1_old,
  ...
  child_->calc_yhat(_agg, _old_weight, _new_weights,
...
void Avg::commit(const Float _old_intercept, const Float _old_weight,
  ...
  if (std::isnan(std::get<2>(_weights))) {
    ...
    for (auto ix : indices_) {
      w_fixed_committed_[ix] =
          eta1_2_null_[ix] * std::get<1>(_weights) + w_fixed_1_[ix];
    }
  }
  ...
  // ------------------------------------------------------------------------
    ...
    for (auto ix : indices_) {
      w_fixed_committed_[ix] =
          eta2_1_null_[ix] * std::get<2>(_weights) + w_fixed_2_[ix];
    }
  }
...
void Avg::deactivate(const Float _old_weight,
  ...
  for (auto it = _begin; it != _end; ++it) {
    ...
    if (count_committed_[ix] == count2_[ix]) {
      ...
      eta1_2_null_[ix] = count1_[ix] / (count_committed_[ix] - count2_[ix]);
    ...
    if (count_committed_[ix] == count1_[ix]) {
      ...
      eta2_1_null_[ix] = count2_[ix] / (count_committed_[ix] - count1_[ix]);
...
Float Avg::evaluate_split(const Float _old_intercept, const Float _old_weight,
                          const std::array<Float, 3>& _weights,
                          const std::vector<containers::Match>::iterator _begin,
...
void Avg::revert(const Float _old_weight) {
  ...
  if (!std::isnan(_old_weight)) {
    child_->calc_etas(enums::Aggregation::avg, update_, _old_weight,
                      indices_current_.unique_integers(), eta1_, eta1_old_,
                      eta2_, eta2_old_);
  }

  if (allow_null_weights_) {
    child_->calc_etas(enums::Aggregation::avg_second_null, update_, _old_weight,
                      indices_current_.unique_integers(), eta1_2_null_,
                      eta1_2_null_old_, w_fixed_1_, w_fixed_1_old_);

    child_->calc_etas(enums::Aggregation::avg_first_null, update_, _old_weight,
                      indices_current_.unique_integers(), eta2_1_null_,
                      eta2_1_null_old_, w_fixed_2_, w_fixed_2_old_);
  }
