...
DecisionTree::DecisionTree(
    ...
    const size_t _peripheral_used, multithreading::Communicator* _comm)
    : comm_(_comm),
...
void DecisionTree::fit(const containers::DataFrameView& _output,
  ...
  root_.reset(new DecisionTreeNode(
      utils::ConditionMaker(hyperparameters().delta_t_, peripheral_used()), 0,
      hyperparameters_, loss_function_, input_, output_, 0.0, &comm()));
...
std::shared_ptr<std::vector<Float>> DecisionTree::transform(
  ...
  for (size_t ix_output = 0; ix_output < _output.nrows(); ++ix_output) {
    ...
    for (size_t i = 0; i < matches.size(); ++i) {
      weights[i] = root_->transform(_output, _input, _subfeatures, matches[i]);
    }
