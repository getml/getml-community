...
DecisionTreeNode::DecisionTreeNode(
    ...
    const std::shared_ptr<const helpers::Schema>& _output, const Float _weight,
...
DecisionTreeNode::DecisionTreeNode(
    ...
    : comm_(nullptr),
      ...
      loss_reduction_(NAN),
      output_(_output),
      weight_(_obj.has("weight_") ? JSON::get_value<Float>(_obj, "weight_")
                                  : NAN) {
  assert_true(input_);
  assert_true(output_);
...
void DecisionTreeNode::add_candidates(
  ...
  auto pairs = loss_function().calc_pairs(
      ...
      weight_, _begin, _last_it, _it, _end);
  ...
  for (const auto& [loss_reduction, new_weights] : pairs) {
    ...
    assert_true(!std::isnan(std::get<1>(new_weights)) ||
                !std::isnan(std::get<2>(new_weights)));

    _candidates->push_back(
        containers::CandidateSplit(loss_reduction, _split, new_weights));
  }
...
void DecisionTreeNode::fit(
  ...
  // ------------------------------------------------------------------------
  // Identify best candidate split.
  // For the relational version, it is the one with the minimal partial loss.
  // For the flat version, it is the ine with the maximal loss reduction.
  ...
  const auto it =
      (_input) ? std::min_element(candidates.begin(), candidates.end(), cmp)
  ...
  *_intercept = std::get<0>(best_split.weights_);
  ...
  child_greater_->fit(_output, _input, _subfeatures, _begin, it_split,
                      _intercept);

  child_smaller_->fit(_output, _input, _subfeatures, it_split, _end,
                      _intercept);
...
void DecisionTreeNode::to_sql(
  ...
  const auto weight =
      std::isnan(weight_) ? "NULL" : io::Parser::to_precise_string(weight_);
...
Float DecisionTreeNode::transform(
  ...
  switch (split_.data_used_) {
    case enums::DataUsed::categorical_input:
      assert_true(_input);
      is_greater =
          utils::Partitioner<enums::DataUsed::categorical_input>::is_greater(
              split_, *_input, _match);
      break;

    case enums::DataUsed::categorical_output:
      is_greater =
          utils::Partitioner<enums::DataUsed::categorical_output>::is_greater(
    ...
    case enums::DataUsed::discrete_input:
      assert_true(_input);
      is_greater =
          utils::Partitioner<enums::DataUsed::discrete_input>::is_greater(
              split_, *_input, _match);
      break;
    ...
    case enums::DataUsed::discrete_output:
      is_greater =
          utils::Partitioner<enums::DataUsed::discrete_output>::is_greater(
              split_, _output, _match);
      break;
    ...
    case enums::DataUsed::numerical_input:
      assert_true(_input);
      is_greater =
          utils::Partitioner<enums::DataUsed::numerical_input>::is_greater(
              split_, *_input, _match);
      break;
    ...
    case enums::DataUsed::numerical_output:
      is_greater =
          utils::Partitioner<enums::DataUsed::numerical_output>::is_greater(
              split_, _output, _match);
      break;
    ...
    case enums::DataUsed::same_units_discrete:
      assert_true(_input);
      is_greater =
          utils::Partitioner<enums::DataUsed::same_units_discrete>::is_greater(
              split_, *_input, _output, _match);
      break;

    case enums::DataUsed::same_units_discrete_is_nan:
      assert_true(_input);
      is_greater = utils::
          Partitioner<enums::DataUsed::same_units_discrete_is_nan>::is_greater(
              split_.column_input_, split_.column_, *_input, _output, _match);
      break;
    ...
    case enums::DataUsed::same_units_numerical:
      assert_true(_input);
      is_greater =
          utils::Partitioner<enums::DataUsed::same_units_numerical>::is_greater(
              split_, *_input, _output, _match);
      break;
    ...
    case enums::DataUsed::same_units_numerical_is_nan:
      assert_true(_input);
      is_greater = utils::
          Partitioner<enums::DataUsed::same_units_numerical_is_nan>::is_greater(
              split_.column_input_, split_.column_, *_input, _output, _match);
      break;

    case enums::DataUsed::subfeatures:
      assert_true(_input);
      is_greater = utils::Partitioner<enums::DataUsed::subfeatures>::is_greater(
          split_, _subfeatures, _match);
      break;

    case enums::DataUsed::text_input:
      assert_true(_input);
      is_greater = utils::Partitioner<enums::DataUsed::text_input>::is_greater(
          split_, *_input, _match);
      break;

    case enums::DataUsed::text_output:
      assert_true(_input);
      is_greater = utils::Partitioner<enums::DataUsed::text_output>::is_greater(
          split_, _output, _match);
      break;
    ...
    case enums::DataUsed::time_stamps_window:
      assert_true(_input);
      is_greater =
          utils::Partitioner<enums::DataUsed::time_stamps_window>::is_greater(
              split_, hyperparameters().delta_t_, *_input, _output, _match);
      break;
...
std::vector<containers::CandidateSplit> DecisionTreeNode::try_all(
  ...
  try_categorical_input(_old_intercept, *_input, _begin, _end, &bins,
                        &candidates);

  try_discrete_input(_old_intercept, *_input, _begin, _end, &bins, &candidates);

  try_numerical_input(_old_intercept, *_input, _begin, _end, &bins,
                      &candidates);

  try_subfeatures(_old_intercept, _subfeatures, _begin, _end, &bins,
                  &candidates);
  ...
  try_same_units_numerical(_old_intercept, *_input, _output, _begin, _end,
                           &bins, &candidates);
  ...
  try_categorical_output(_old_intercept, _output, _begin, _end, &bins,
                         &candidates);
  ...
  try_text_output(_old_intercept, _output, _begin, _end, &bins, &candidates);
    ...
    for (size_t input_col = 0; input_col < _input.num_numericals();
      ...
      const bool is_ts =
          _output.numerical_unit(output_col).find("time stamp") !=
              std::string::npos &&
          _output.numerical_name(output_col).find("$GETML_ROWID") ==
              std::string::npos;
...
void DecisionTreeNode::try_text_input(
  ...
  for (size_t j = 0; j < _input.num_text(); ++j) {
    ...
    try_text_input_single_word(j, _old_intercept, _input, _begin, _end, _bins,
                               _candidates);

    // ----------------------------------------------------------------

    try_text_input_multiple_words(j, begin_ix, _old_intercept, _input, _begin,
                                  _end, _bins, _candidates);
...
void DecisionTreeNode::try_text_output(
  ...
  for (size_t j = 0; j < _output.num_text(); ++j) {
    ...
    try_text_output_single_word(j, _old_intercept, _output, _begin, _end, _bins,
                                _candidates);

    // ----------------------------------------------------------------

    try_text_output_multiple_words(j, begin_ix, _old_intercept, _output, _begin,
                                   _end, _bins, _candidates);
