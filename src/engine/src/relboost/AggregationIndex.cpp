...
const Float AggregationIndex::get_count(const Int _ix_agg) const {
  ...
  for (auto it = begin; it != end; ++it) {
    ...
    const bool use_this_sample =
        ...
        time_stamp_output_in_range(input_table_.df().time_stamp(ix_input),
...
std::shared_ptr<std::vector<Float>> AggregationIndex::make_sample_weights(
  ...
  for (size_t i = 0; i < _sample_weights_parent->size(); ++i) {
    ...
    for (auto it = begin; it != end; ++it) {
      ...
      const Float time_stamp_input = input_table_.df().time_stamp(ix_input);

      const Float upper_time_stamp =
          input_table_.df().upper_time_stamp(ix_input);

      const bool use_this_sample = time_stamp_output_in_range(
          time_stamp_input, upper_time_stamp, output_table_.time_stamp(i));

      if (use_this_sample) {
        auto ix_input_tr =
            transform_ix_agg(static_cast<Int>(ix_input), *input_map_);

        if (ix_input_tr >= 0) {
          assert_true(ix_input_tr < input_table_.nrows());

          assert_true(ix_input == input_table_.rows()[ix_input_tr]);
...
const std::vector<Int> AggregationIndex::transform(
  ...
  for (auto it = begin; it != end; ++it) {
    ...
    const bool use_this_sample =
        time_stamp_output_in_range(time_stamp_input, upper_time_stamp,
                                   output_table_.df().time_stamp(ix_agg));

    if (use_this_sample) {
      auto ix_agg_tr = transform_ix_agg(static_cast<Int>(ix_agg), *output_map_);
