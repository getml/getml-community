...
#include "relboost/ensemble/TableHolderParams.hpp"
...
DecisionTreeEnsemble::DecisionTreeEnsemble(const Poco::JSON::Object &_obj)
  ...
  for (size_t i = 0; i < trees_objects.size(); ++i) {
    trees().push_back(decisiontrees::DecisionTree(
        impl().hyperparameters_, loss_function_,
...
void DecisionTreeEnsemble::fit(const FitParams &_params) {
  if (num_features() != 0) {
    throw std::runtime_error("Relboost model has already been fitted!");
  }
...
DecisionTreeEnsemble::fit_candidate_features(
  ...
  for (size_t ix_table_used = 0;
    ...
    aggregations.push_back(std::make_shared<aggregations::Avg>(
        _loss_function, matches, input_table, output_table,
        hyperparameters().allow_null_weights_, &comm()));
    ...
    for (auto &agg : aggregations) {
      ...
      new_candidate.fit(output_table, input_table, subfeatures, matches.begin(),
                        matches.end());

      auto new_predictions =
          new_candidate.transform(output_table, input_table, subfeatures);
      ...
      const auto new_loss_reduction = _loss_function->evaluate_tree(
          new_candidate.update_rate(), *new_predictions);

      candidates.emplace_back(std::make_tuple(std::move(new_candidate),
                                              new_loss_reduction,
