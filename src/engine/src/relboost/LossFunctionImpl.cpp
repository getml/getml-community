...
std::pair<Float, std::array<Float, 3>> LossFunctionImpl::calc_all(
  ...
  *_loss_old = apply_xgboost(sum_g1 + sum_g2, sum_h1 + sum_h2).first;
  ...
  if (n1 < m || n2 < m) {
    return std::make_pair(static_cast<Float>(NAN),
...
std::pair<Float, std::array<Float, 3>> LossFunctionImpl::calc_diff(
    ...
    const std::vector<containers::Match>::iterator _split_begin,
    const std::vector<containers::Match>::iterator _split_end,
    const std::vector<containers::Match>::iterator _end, const Float _loss_old,
    std::array<Float, 6>* _sufficient_stats,
    multithreading::Communicator* _comm) const {
  // ------------------------------------------------------------------------
  ...
  auto n_diff = static_cast<Float>(std::distance(_split_begin, _split_end));
...
Float LossFunctionImpl::calc_loss(const Float _update_rate,
  ...
  for (size_t ix : sample_index_) {
    ...
    loss += (g_[ix] * p + 0.5 * h_[ix] * p * p) * (*sample_weights_)[ix];
  }
...
std::pair<Float, std::array<Float, 3>> LossFunctionImpl::calc_pair_avg_null(
    const enums::Aggregation _agg, const enums::Update _update,
    const Float _old_weight, const std::vector<size_t>& _indices_current,
    const std::vector<Float>& _eta, const std::vector<Float>& _eta_old,
    const std::vector<Float>& _w_fixed, const std::vector<Float>& _w_fixed_old,
    const std::vector<Float>& _yhat_committed,
    std::array<Float, 8>* _sufficient_stats,
    multithreading::Communicator* _comm) const {
  // ------------------------------------------------------------------------

  calc_sufficient_stats_avg_null(_update, _old_weight, _indices_current, _eta,
                                 _eta_old, _w_fixed, _w_fixed_old,
                                 _yhat_committed, _sufficient_stats);
  ...
  utils::Reducer::reduce<8>(std::plus<Float>(), &sufficient_stats_global,
                            _comm);
...
std::pair<Float, std::array<Float, 3>> LossFunctionImpl::calc_pair_non_null(
    const enums::Update _update, const Float _old_weight,
  ...
  calc_sufficient_stats_non_null(_update, _old_weight, _indices_current, _eta1,
  ...
  utils::Reducer::reduce<13>(std::plus<Float>(), &sufficient_stats_global,
                             _comm);
...
Float LossFunctionImpl::calc_regularization_reduction(
  ...
  if (std::isnan(new_weight1)) {
    regularization += _old_weight * _old_weight - new_weight2 * new_weight2;
  } else if (std::isnan(new_weight2)) {
    regularization += _old_weight * _old_weight - new_weight1 * new_weight1;
  } else {
    regularization += _old_weight * _old_weight - new_weight1 * new_weight1 -
...
void LossFunctionImpl::calc_sufficient_stats_avg_null(
    const enums::Update _update, const Float _old_weight,
    const std::vector<size_t>& _indices_current, const std::vector<Float>& _eta,
    const std::vector<Float>& _eta_old, const std::vector<Float>& _w_fixed,
  ...
  if (_update == enums::Update::calc_all) {
    std::fill(_sufficient_stats->begin(), _sufficient_stats->end(), 0.0);
  }
  ...
  for (const auto ix : _indices_current) {
    ...
    assert_true(_update != enums::Update::calc_all || _eta_old[ix] == 0.0);
  ...
  if (_update == enums::Update::calc_all) {
    ...
    for (const auto ix : _indices_current) {
      h_w_const_arr[0] -=
          h_[ix] * (_w_fixed[ix] - _yhat_committed[ix]) * sample_weights(ix);
    }
  }
  ...
  for (const auto ix : _indices_current) {
    ...
    const auto d_w_fixed2 =
        _w_fixed[ix] * _w_fixed[ix] - _w_fixed_old[ix] * _w_fixed_old[ix];

    h_w_const_arr[1] -= h_[ix] * d_w_fixed * d_eta * sample_weights(ix);

    loss_w_fixed +=
        (g_[ix] * d_w_fixed + 0.5 * h_[ix] * d_w_fixed2) * sample_weights(ix);
  }
  ...
  for (const auto ix : _indices_current) {
    ...
    const auto d_eta2 = _eta[ix] * _eta[ix] - _eta_old[ix] * _eta_old[ix];
...
void LossFunctionImpl::calc_sufficient_stats_non_null(
  ...
  if (_update == enums::Update::calc_all) {
    ...
    for (const auto ix : _indices_current) {
      ...
      loss_w_fixed +=
          (g_[ix] + 0.5 * h_[ix] * w_fixed) * w_fixed * sample_weights(ix);
    }
  }
  ...
  for (const auto ix : _indices_current) {
    ...
    h_w_const_arr[1] -= h_[ix] * w_fixed * d_eta1 * sample_weights(ix);
    h_w_const_arr[2] -= h_[ix] * w_fixed * d_eta2 * sample_weights(ix);
  }
  ...
  for (const auto ix : _indices_current) {
    ...
    const auto d_eta11 = _eta1[ix] * _eta1[ix] - _eta1_old[ix] * _eta1_old[ix];

    const auto d_eta22 = _eta2[ix] * _eta2[ix] - _eta2_old[ix] * _eta2_old[ix];

    const auto d_eta12 = _eta1[ix] * _eta2[ix] - _eta1_old[ix] * _eta2_old[ix];
...
void LossFunctionImpl::calc_sums(const std::vector<size_t>& _sample_index,
                                 const std::vector<Float>& _sample_weights,
                                 Float* _sum_g, Float* _sum_h,
...
Float LossFunctionImpl::calc_update_rate(
    const std::vector<Float>& _yhat_old, const std::vector<Float>& _predictions,
    multithreading::Communicator* _comm) const {
  // ------------------------------------------------------------------------
...
void LossFunctionImpl::calc_yhat(const Float _old_weight,
  ...
  for (auto ix : _indices) {
    (*_yhat)[ix] = _yhat_committed[ix] + _eta1[ix] * std::get<1>(_new_weights) +
...
void LossFunctionImpl::calc_yhat_avg_null(
    ...
    const std::vector<size_t>& _indices, const std::vector<Float>& _eta,
    const std::vector<Float>& _w_fixed, std::vector<Float>* _yhat) const {
  if (std::isnan(std::get<2>(_new_weights))) {
    assert_true(!std::isnan(std::get<1>(_new_weights)));

    for (auto ix : _indices) {
      (*_yhat)[ix] = _eta[ix] * std::get<1>(_new_weights) + _w_fixed[ix];
    }
  } else if (std::isnan(std::get<1>(_new_weights))) {
    assert_true(!std::isnan(std::get<2>(_new_weights)));

    for (auto ix : _indices) {
      (*_yhat)[ix] = _eta[ix] * std::get<2>(_new_weights) + _w_fixed[ix];
    }
  } else {
    assert_true(false && "Either the first or the second weight must be NAN!");
  }
}

// ----------------------------------------------------------------------------

Float LossFunctionImpl::commit(const std::vector<size_t>& _indices,
  ...
  for (size_t ix : _indices) {
    ...
    sum_h_yhat +=
        sample_weights(ix) * (_yhat[ix] - (*_yhat_committed)[ix]) * h_[ix];
  }
...
void LossFunctionImpl::transform(
  ...
  assert_true(static_cast<int>(_weights.size()) == std::distance(_begin, _end));
...
void LossFunctionImpl::update_yhat_old(const Float _update_rate,
                                       const std::vector<Float>& _predictions,
                                       std::vector<Float>* _yhat_old) const {
  assert_true(_predictions.size() == _yhat_old->size());

  std::transform(_yhat_old->begin(), _yhat_old->end(), _predictions.begin(),
                 _yhat_old->begin(),
