#include "multirel/descriptors/SameUnits.hpp"
...
bool SameUnits::is_ts(const containers::DataFrameView& _population,
  ...
  const auto contains_time_stamp = unit.find("time stamp") != std::string::npos;
...
descriptors::SameUnitsContainer SameUnits::json_arr_to_same_units(
  ...
  for (size_t j = 0; j < _json_arr.size(); ++j) {
    ...
    column_to_be_aggregated1.data_used =
        JSON::int_to_data_used(JSON::get_value<int>(child1, "data_used"));
    ...
    column_to_be_aggregated2.data_used =
        JSON::int_to_data_used(JSON::get_value<int>(child2, "data_used"));
    ...
    same_units.push_back(
        std::make_tuple(column_to_be_aggregated1, column_to_be_aggregated2));
...
Poco::JSON::Array SameUnits::same_units_to_json_arr(
  ...
  for (const std::tuple<descriptors::ColumnToBeAggregated,
                        descriptors::ColumnToBeAggregated>& same_unit :
       _same_units) {
    ...
    child1.set("ix_column_used", std::get<0>(same_unit).ix_column_used);

    child1.set("data_used",
               JSON::data_used_to_int(std::get<0>(same_unit).data_used));

    child1.set("ix_perip_used", std::get<0>(same_unit).ix_perip_used);
    ...
    child2.set("ix_column_used", std::get<1>(same_unit).ix_column_used);

    child2.set("data_used",
               JSON::data_used_to_int(std::get<1>(same_unit).data_used));

    child2.set("ix_perip_used", std::get<1>(same_unit).ix_perip_used);
