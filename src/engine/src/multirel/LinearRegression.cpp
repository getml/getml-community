...
void LinearRegression::fit(const std::vector<Float>& _new_feature,
  ...
  std::for_each(mean_residuals.begin(), mean_residuals.end(),
  ...
  for (size_t i = 0; i < _residuals.size(); ++i) {
    for (size_t j = 0; j < _residuals[i].size(); ++j) {
      cov_new_feature[i] += (_residuals[i][j] - mean_residuals[i]) *
  ...
  std::for_each(cov_new_feature.begin(), cov_new_feature.end(),
...
LinearRegression LinearRegression::from_json_obj(
  ...
  linear_regression.slopes_ =
      JSON::array_to_vector<Float>(JSON::get_array(_obj, "update_rates_1_"));

  linear_regression.intercepts_ =
      JSON::array_to_vector<Float>(JSON::get_array(_obj, "update_rates_2_"));
