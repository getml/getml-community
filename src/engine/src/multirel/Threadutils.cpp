...
void Threadutils::fit_ensemble(const ThreadutilsFitParams& _params) {
  try {
    ...
    _params.ensemble_->fit(table_holder, _params.word_indices_, _params.logger_,
...
void Threadutils::transform_ensemble(
  ...
  try {
    ...
    const auto subpredictions = SubtreeHelper::make_predictions(
        ...
        _params.ensemble_.subensembles_sum(), _params.logger_, _params.comm_);
    ...
    for (size_t i = 0; i < _params.index_.size(); ++i) {
      ...
      const auto new_feature =
          _params.ensemble_.transform(table_holder, subfeatures, ix, &impl);
