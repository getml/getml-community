...
DecisionTree::DecisionTree(
    ...
    const size_t _ix_column_used, const descriptors::SameUnits &_same_units,
    std::mt19937 *_random_number_generator, multithreading::Communicator *_comm)
    : impl_(_tree_hyperparameters) {
...
void DecisionTree::from_json_obj(const Poco::JSON::Object &_json_obj) {
  ...
  impl()->output_ = std::make_shared<const helpers::Schema>(
      helpers::Schema::from_json(*JSON::get_object(_json_obj, "output_")));
...
void DecisionTree::handle_fast_prop_importances(
  ...
  const auto fast_prop_input =
      ix_perip_used() < _fast_prop_container.size()
          ? _fast_prop_container.subcontainers(ix_perip_used())
...
std::string DecisionTree::to_sql(
  ...
  sql << sql_maker.select_statement(_feature_prefix, input(), output(),
  ...
  if (output_has_prop) {
    sql << _sql_dialect_generator->make_subfeature_joins(
        _feature_prefix, ix_perip_used(), "t1", "_PROPOSITIONALIZATION");
  }
  ...
  root()->to_sql(_categories, _vocabulary.population(),
...
std::shared_ptr<const std::vector<Float>> DecisionTree::transform(
  ...
  for (size_t ix_x_popul = 0; ix_x_popul < _population.nrows(); ++ix_x_popul) {
    ...
    if (aggregation_type() != aggregations::AggregationType::Count::type()) {
    ...
    const auto is_deactivated = [this, _population, _peripheral,
      ...
      return !root()->transform(_population, _peripheral, _subfeatures, _m);
    };
    ...
    (*yhat)[ix_x_popul] = aggregation->aggregate(match_ptrs, skip, time_stamp);
