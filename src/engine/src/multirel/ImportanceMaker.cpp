...
void ImportanceMaker::add(const helpers::Schema& _input,
  ...
  switch (_data_used) {
    case enums::DataUsed::x_perip_categorical: {
      ...
      const auto desc = helpers::ColumnDescription(
          peripheral(), _input.name(), _input.categorical_name(_column));
    ...
    case enums::DataUsed::x_popul_categorical: {
      ...
      const auto desc = helpers::ColumnDescription(
          population(), _output.name(), _output.categorical_name(_column));
    ...
    case enums::DataUsed::x_perip_discrete: {
      ...
      const auto desc = helpers::ColumnDescription(
          peripheral(), _input.name(), _input.discrete_name(_column));
    ...
    case enums::DataUsed::x_popul_discrete: {
      ...
      const auto desc = helpers::ColumnDescription(
          population(), _output.name(), _output.discrete_name(_column));
    ...
    case enums::DataUsed::x_perip_numerical: {
      ...
      const auto desc = helpers::ColumnDescription(
          peripheral(), _input.name(), _input.numerical_name(_column));
    ...
    case enums::DataUsed::x_popul_numerical: {
      ...
      const auto desc = helpers::ColumnDescription(
          population(), _output.name(), _output.numerical_name(_column));
    ...
    case enums::DataUsed::x_perip_text: {
      ...
      const auto desc = helpers::ColumnDescription(peripheral(), _input.name(),
    ...
    case enums::DataUsed::x_popul_text: {
      ...
      const auto desc = helpers::ColumnDescription(population(), _output.name(),
    ...
    case enums::DataUsed::same_unit_categorical: {
      ...
      assert_true(_column < _same_units.same_units_categorical_->size());

      const auto& upair = _same_units.same_units_categorical_->at(_column);

      add(_input, _output, std::get<0>(upair).data_used,
          std::get<0>(upair).ix_column_used, _same_units, _value * 0.5);

      add(_input, _output, std::get<1>(upair).data_used,
          std::get<1>(upair).ix_column_used, _same_units, _value * 0.5);
    ...
    case enums::DataUsed::same_unit_discrete: {
      ...
      assert_true(_column < _same_units.same_units_discrete_->size());

      const auto& upair = _same_units.same_units_discrete_->at(_column);
    ...
    case enums::DataUsed::same_unit_numerical: {
      ...
      assert_true(_column < _same_units.same_units_numerical_->size());

      const auto& upair = _same_units.same_units_numerical_->at(_column);
    ...
    case enums::DataUsed::time_stamps_window: {
      const auto desc1 = helpers::ColumnDescription(peripheral(), _input.name(),
                                                    _input.time_stamps_name());

      const auto desc2 = helpers::ColumnDescription(
          population(), _output.name(), _output.time_stamps_name());
