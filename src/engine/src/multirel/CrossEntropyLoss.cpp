...
std::vector<std::vector<Float>> CrossEntropyLoss::calculate_residuals(
  ...
  for (size_t j = 0; j < _y.num_targets(); ++j) {
    ...
    for (size_t i = 0; i < _y.nrows(); ++i) {
      ...
      residuals[j][i] = _y.target(i, j) - logistic_function(_yhat_old[j][i]);
    }
  }
...
std::vector<Float> CrossEntropyLoss::calculate_update_rates(
  ...
  for (size_t j = 0; j < _y.num_targets(); ++j) {
    ...
    for (size_t i = 0; i < _y.nrows(); ++i) {
      ...
      g_times_p[j] += (logistic - _y.target(i, j)) * _predictions[j][i];

      h_times_p_squared[j] +=
          logistic * (1.0 - logistic) * _predictions[j][i] * _predictions[j][i];
    }
  }
