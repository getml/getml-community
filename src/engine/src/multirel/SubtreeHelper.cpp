...
void SubtreeHelper::fit_subensembles(
  ...
  for (size_t i = 0; i < num_tables; ++i) {
    if (_table_holder->subtables().at(i)) {
      ...
      subensembles_avg.at(i).reset(
          new DecisionTreeEnsemble(hyperparameters, peripheral, joined_table));

      subensembles_sum.at(i).reset(
          new DecisionTreeEnsemble(hyperparameters, peripheral, joined_table));
    }
  }
  ...
  for (size_t i = 0; i < num_tables; ++i) {
    if (subensembles_avg[i]) {
      fit_subensemble<aggregations::AggregationType::Avg>(
          _table_holder, _word_indices, _logger, rows_map,
          _ensemble.hyperparameters(), i, _opt, _comm,
  ...
  for (size_t i = 0; i < size; ++i) {
    ...
    auto impl = containers::Optional<aggregations::AggregationImpl>(
        new aggregations::AggregationImpl(
            _table_holder.subtables()[i]->main_tables().at(0).nrows()));
