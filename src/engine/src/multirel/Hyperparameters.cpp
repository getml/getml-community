#include "multirel/descriptors/Hyperparameters.hpp"
    ...
    : aggregations_(JSON::array_to_vector<std::string>(
      ...
      include_categorical_(
          _json_obj.has("include_categorical_")
              ...
              : true),  // TODO: Remove
      loss_function_(JSON::get_value<std::string>(_json_obj, "loss_function_")),
      min_df_(
          _json_obj.has("min_df_")
              ? JSON::get_value<size_t>(_json_obj, "min_df_")
              : static_cast<size_t>(30)),  // TODO: Check inserted for backwards
                                           // compatability. Remove later.
      num_features_(JSON::get_value<size_t>(_json_obj, "num_features_")),
      num_subfeatures_(JSON::get_value<size_t>(_json_obj, "num_subfeatures_")),
      ...
      sampling_factor_(JSON::get_value<Float>(_json_obj, "sampling_factor_")),
      ...
      silent_(_json_obj.has("silent_")  // for backwards compatability
      ...
      vocab_size_(
          _json_obj.has("vocab_size_")
              ...
              : static_cast<size_t>(500))  // TODO: Check inserted for backwards
                                           // compatability. Remove later.
{}
...
Poco::JSON::Object::Ptr Hyperparameters::to_json_obj() const {
  ...
  if (propositionalization_) {
    obj->set("propositionalization_", propositionalization_->to_json_obj());
  }
