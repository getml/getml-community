...
void CandidateTreeBuilder::add_counts(
    ...
    const Int _ix_perip_used, std::mt19937 *_random_number_generator,
  ...
  for (auto &agg : _hyperparameters.aggregations_) {
    ...
    _candidate_trees->push_back(decisiontrees::DecisionTree(
        agg, _hyperparameters.tree_hyperparameters_, _ix_perip_used,
        enums::DataUsed::not_applicable,  // data_used
        0,                                // _ix_column_used - not important
        _same_units[_ix_perip_used], _random_number_generator, _comm));
  }
}
...
void CandidateTreeBuilder::add_count_distincts(
    ...
    std::list<decisiontrees::DecisionTree> *_candidate_trees) {
  for (auto &agg : _hyperparameters.aggregations_) {
    if (agg != "COUNT DISTINCT" && agg != "COUNT MINUS COUNT DISTINCT") {
      continue;
    }

    for (auto data_used : {enums::DataUsed::x_perip_categorical,
                           enums::DataUsed::x_perip_discrete}) {
      size_t ncols = get_ncols(_table_holder.peripheral_tables(), _same_units,
                               _ix_perip_used, data_used);

      for (size_t ix_column_used = 0; ix_column_used < ncols;
           ++ix_column_used) {
        _candidate_trees->push_back(decisiontrees::DecisionTree(
            agg, _hyperparameters.tree_hyperparameters_, _ix_perip_used,
            data_used, ix_column_used, _same_units[_ix_perip_used],
            _random_number_generator, _comm));
      }
    }
  }
}
...
void CandidateTreeBuilder::add_other_aggs(
  ...
  assert_true(static_cast<size_t>(_ix_perip_used) <
  ...
  for (auto &agg : _hyperparameters.aggregations_) {
    ...
    if (skip_first_last(agg,
                        _table_holder.peripheral_tables().at(_ix_perip_used))) {
      continue;
    }

    for (auto data_used :
         {enums::DataUsed::x_perip_numerical, enums::DataUsed::x_perip_discrete,
      ...
      for (size_t ix_column_used = 0; ix_column_used < ncols;
        ...
        const bool comparison_only = is_comparison_only(
            _table_holder, data_used, _ix_perip_used, ix_column_used);
        ...
        data_used = to_ts(_table_holder, _same_units, data_used, _ix_perip_used,
                          ix_column_used);
...
void CandidateTreeBuilder::add_subfeature_aggs(
  ...
  const bool all_propositionalization =
      std::all_of(subholder.propositionalization().begin(),
                  subholder.propositionalization().end(), is_true);
  ...
  for (auto &agg : _hyperparameters.aggregations_) {
    ...
    for (Int ix_column_used = 0;
      ...
      _candidate_trees->push_back(decisiontrees::DecisionTree(
          agg, _hyperparameters.tree_hyperparameters_, _ix_perip_used,
          enums::DataUsed::x_subfeature, ix_column_used,
          _same_units[_ix_perip_used], _random_number_generator, _comm));
    }
  }
}
...
std::list<decisiontrees::DecisionTree> CandidateTreeBuilder::build_candidates(
  ...
  auto candidate_trees =
      build_candidate_trees(_table_holder, _same_units, _hyperparameters,
                            _random_number_generator, _comm);
  ...
  if (_hyperparameters.round_robin_ && _ix_feature != -1) {
    ...
    randomly_remove_candidate_trees(_hyperparameters.share_aggregations_,
                                    _random_number_generator, _comm,
                                    &candidate_trees);
  }
...
CandidateTreeBuilder::build_candidate_trees(
  ...
  for (size_t ix_perip_used = 0; ix_perip_used < num_perips; ++ix_perip_used) {
    // -----------------------------------------------------
    ...
    add_counts(_table_holder, _same_units, _hyperparameters,
               static_cast<Int>(ix_perip_used), _random_number_generator, _comm,
               &candidate_trees);
    ...
    add_count_distincts(_table_holder, _same_units, _hyperparameters,
                        static_cast<Int>(ix_perip_used),
                        _random_number_generator, _comm, &candidate_trees);

    // ------------------------------------------------------------------
    // Now we apply all of the aggregations.

    add_other_aggs(_table_holder, _same_units, _hyperparameters,
                   static_cast<Int>(ix_perip_used), _random_number_generator,
                   _comm, &candidate_trees);

    // ------------------------------------------------------------------
    // If applicable, add aggregations over the subfeatures

    if (_table_holder.subtables().at(ix_perip_used)) {
      add_subfeature_aggs(_table_holder, _same_units, _hyperparameters,
                          static_cast<Int>(ix_perip_used),
                          _random_number_generator, _comm, &candidate_trees);
    }
...
size_t CandidateTreeBuilder::get_ncols(
    ...
    const std::vector<descriptors::SameUnits> &_same_units,
    const size_t _ix_perip_used, const enums::DataUsed _data_used) {
...
bool CandidateTreeBuilder::is_comparison_only(
    const decisiontrees::TableHolder &_table_holder,
    const enums::DataUsed _data_used, const size_t _ix_perip_used,
...
void CandidateTreeBuilder::randomly_remove_candidate_trees(
    const Float _share_aggregations, std::mt19937 *_random_number_generator,
    multithreading::Communicator *_comm,
    std::list<decisiontrees::DecisionTree> *_candidate_trees) {
  // ------------------------------------------------
...
enums::DataUsed CandidateTreeBuilder::to_ts(
  ...
  const auto peripheral = _table_holder.peripheral_tables().at(_ix_perip_used);
  ...
  switch (_data_used) {
    case enums::DataUsed::same_unit_numerical: {
      ...
      const auto is_ts =
          _same_units.at(_ix_perip_used)
              .is_ts(population, peripheral, *same_unit, _ix_column_used);
