...
void SapHana::execute(const std::string& _sql) {
  ...
  for (const auto& query : queries) {
    goutils::SapHana::exec(user_, password_, host_, port_, default_schema_,
                           ping_interval_, query);
  }
}
...
std::vector<std::string> SapHana::get_colnames(
  ...
  const auto [colnames, _] =
      goutils::SapHana::colnames(user_, password_, host_, port_,
                                 default_schema_, ping_interval_, sql_query);
...
std::vector<io::Datatype> SapHana::get_coltypes(
  ...
  const auto [typenames, _] = goutils::SapHana::coltypes(
      user_, password_, host_, port_, default_schema_, ping_interval_, _table);
...
Poco::JSON::Object SapHana::get_content(const std::string& _tname,
...
void SapHana::read(const std::string& _table, const size_t _skip,
                   io::Reader* _reader) {
  while (!_reader->eof()) {
    ...
    goutils::SapHana::load(user_, password_, host_, port_, default_schema_,
                           ping_interval_, _table, nrows, ncols, ptrs.data());
  }
}
