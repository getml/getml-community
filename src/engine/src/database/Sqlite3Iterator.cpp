...
Sqlite3Iterator::Sqlite3Iterator(
    ...
    : Sqlite3Iterator(_db, make_sql(_colnames, _tname, _where),
                      _read_write_lock, _time_formats)
...
Float Sqlite3Iterator::get_double() {
  ...
  if (val == 0.0) {
    ...
    if (txt) {
      const auto str = std::string(reinterpret_cast<const char*>(txt));
...
std::string Sqlite3Iterator::make_sql(const std::vector<std::string>& _colnames,
